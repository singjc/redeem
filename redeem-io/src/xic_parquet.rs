...
///
/// This helper is meant for fast diagnostics when each `*.xic` file is
/// expected to contain chromatograms for exactly one run. It avoids scanning
/// the entire parquet file the way [`list_run_ids`] does.
pub fn first_run_id(path: &Path) -> Result<Option<u64>> {
    ...
    if let Some(row) = iter.next() {
        let row = row?;
        return Ok(Some(
            XicParquetReader::get_i64(&row, idx_run, "RUN_ID")? as u64
        ));
    }
    Ok(None)
}
