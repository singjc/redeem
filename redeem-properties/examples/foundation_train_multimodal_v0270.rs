//! v0.27 RT-specialist + contextual-fragment architecture warm-started from frozen v0.26.1.
//!
...
//!
//! All unchanged v0.26.1 namespaces are restored exactly; only the new RT
//! specialist and contextual fragment decoder are freshly initialized. Historical
//! VALIDATION and TEST remain outside optimization/model selection.
...
use redeem_properties::foundation::{
    ...
    foundation_ms2_loss, foundation_multimodal_ms2_loss_v0270,
    foundation_multimodal_relation_margin_loss_v0270, foundation_peptidoform_neutral_mass,
    ...
    FoundationDiffusionConfig, FoundationDiffusionVocabulary, FoundationFragmentContextBatchV0270,
    ...
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0270, FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0270,
    FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0270, FOUNDATION_OPEN_PTM_MASS_SCALE_DA,
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0270 RUN_V0260.yaml OUTPUT_DIR PARENT_V0261_CHECKPOINT [max_epochs=30] [batch_size=32] [patience=5] [min_delta=0.002] [seed=20260925] [learning_rate=1e-4] [mode=train|resume|finalize]"
        );
    }
    ...
    let (resume_training, finalize_only) = match run_mode {
        ...
        "finalize" => (false, true),
        other => anyhow::bail!(
            "unsupported v0.27 run mode {other:?}; expected train, resume, or finalize"
        ),
    };
    ...
    if finalize_only || resume_training {
        if !output_root.is_dir() {
            anyhow::bail!(
                "v0.27 {run_mode} mode requires an existing model output directory: {:?}",
                output_root
            );
        }
        ...
        for checkpoint in required_checkpoints {
            for name in [
                ...
                if !path.is_file() {
                    anyhow::bail!("v0.27 {run_mode} mode is missing checkpoint file: {path:?}");
                }
            }
        }
    } else if output_root.exists() {
        anyhow::bail!("v0.27 output directory already exists: {:?}", output_root);
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.27 requires a CUDA device")?;
    ...
    if parent_metadata.version != 261 {
        anyhow::bail!(
            "v0.27 requires the frozen v0.26.1 parent metadata version 261, observed {}",
            parent_metadata.version
        );
    }
    // v0.27 changes only RT specialization and the forward-MS2 decoder. Preserve
    // the accepted v0.26.1 shared/inverse/CCS configuration exactly.
    let forward_config = parent_metadata.forward_config;
    let inverse_config = parent_metadata.inverse_config;
    forward_config.validate().map_err(anyhow::Error::msg)?;
    inverse_config.validate().map_err(anyhow::Error::msg)?;
    let v0270_config = PeptideFoundationMultimodalV0270Config::fixed(
        forward_config.clone(),
        inverse_config.clone(),
    )?;

    // The v0.27 prepare step excludes peptides that cannot be represented by
    ...
    if let Some((record_index, sequence, sequence_len)) = first_overlength {
        anyhow::bail!(
            "v0.27 prepared benchmark contains {prepared_overlength_records} peptide(s) longer than architecture max_sequence_len={}; first record_index={record_index} length={sequence_len} sequence={sequence}",
    ...
    let model = PeptideFoundationMultimodalV0270Model::new(v0270_config.clone(), vb)?;

    // Selectively restore every unchanged v0.26.1 namespace. The two new v0.27
    // branches are intentionally absent from the parent and remain freshly initialized.
    let warm_start_report = load_v0261_unchanged_variables(
        &varmap,
        &parent_checkpoint.join("model.safetensors"),
        &device,
    )?;
    let warm_start_description = format!(
        "v0261_unchanged_namespaces_from={}",
        parent_checkpoint.display()
    );
    let alignment_initialization = "v0261_frozen_checkpoint".to_string();
    ...
    println!("v0270_version\tv0.27-rt-specialist-contextual-fragment-tokens");
    println!("objective\tv0270_rt_specialist_contextual_fragment_tokens_openptm32");
    println!("architecture\t{}", FOUNDATION_MULTIMODAL_ARCHITECTURE_V0270);
    ...
    println!("parent_checkpoint_weights_loaded\ttrue_unchanged_namespaces_only");
    println!("random_initialization\tnew_rt_and_fragment_branches_only");
    println!(
    ...
    println!("ccs_physics_path\tpreserved_exactly_from_v0261");
    ...
    let metadata = |completed_steps| UnifiedPilotMetadata {
        version: 270,
        objective: "v0270_rt_specialist_contextual_fragment_tokens_openptm32".into(),
        ...
        relation_margin: FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0270,
        property_adapter_hidden: FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0270,
        ...
        v0270_config: v0270_config.clone(),
    ...
    if finalize_only {
        ...
        if best_metadata.version != 270 {
            anyhow::bail!(
                "v0.27 finalize mode expected checkpoint metadata version 270, observed {}",
                best_metadata.version
            );
        }
        if best_metadata.corpus_fingerprint != expected_corpus_fingerprint {
            anyhow::bail!(
                "v0.27 finalize corpus fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
            anyhow::bail!(
                "v0.27 finalize benchmark fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.batch_size != batch_size {
            anyhow::bail!(
                "v0.27 finalize batch_size mismatch: checkpoint={} requested={batch_size}",
                best_metadata.batch_size
            );
        }
        if best_metadata.seed != seed {
            anyhow::bail!(
                "v0.27 finalize seed mismatch: checkpoint={} requested={seed}",
                best_metadata.seed
            );
        }
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore best v0.27 model {best_model:?}"))?;
        ...
        println!("v0270_finalize_only\ttrue");
        println!("v0270_finalize_source_checkpoint\t{}", best_dir.display());
        println!("v0270_finalize_best_step\t{best_step}");
        println!(
            "v0270_finalize_best_epoch\t{}",
            best_epoch
        ...
        println!("v0270_finalize_complete\tbest_step={best_step}");
    ...
    let (
        ...
        for (label, checkpoint_metadata) in [
            ...
            validate_v0270_training_checkpoint_metadata(
                label,
                checkpoint_metadata,
                &expected_corpus_fingerprint,
                &expected_benchmark_fingerprint,
        ...
        if initial_metadata.completed_steps != 0 {
            anyhow::bail!(
                "v0.27 resume expected initial checkpoint at step 0, observed {}",
        ...
        {
            anyhow::bail!(
                "v0.27 resume requires latest checkpoint at a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
        ...
        {
            anyhow::bail!(
                "v0.27 resume requires best checkpoint at a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                best_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps > latest_metadata.completed_steps {
            anyhow::bail!(
                "v0.27 resume best checkpoint step {} is newer than latest step {}",
                best_metadata.completed_steps,
                latest_metadata.completed_steps
            );
        }
        ...
        varmap
            .load(&initial_model)
            .with_context(|| format!("failed to restore v0.27 initial model {initial_model:?}"))?;
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore v0.27 best model {best_model:?}"))?;
        ...
        varmap
            .load(&latest_model)
            .with_context(|| format!("failed to restore v0.27 latest model {latest_model:?}"))?;
        let latest_optimizer = latest_dir.join("optimizer.safetensors");
        optimizer
            .load_safetensors(&latest_optimizer)
            .with_context(|| {
                format!("failed to restore v0.27 optimizer state {latest_optimizer:?}")
            })?;
        ...
        if stale_epochs >= patience {
            println!(
                "v0270_resume_already_early_stopped\tcompleted_epoch={completed_epoch}\tbest_epoch={best_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
            );
        }
        println!(
            "v0270_resume\tlatest_epoch={completed_epoch}\tlatest_step={global_step}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstale_epochs={stale_epochs}"
        );
        (
            initial_metrics,
            global_step,
    ...
    if start_epoch > max_epochs {
        println!(
            "v0270_training_budget_complete\tstart_epoch={start_epoch}\tmax_epochs={max_epochs}\tlatest_step={global_step}"
        );
    } else if stopped_early {
        println!(
            "v0270_training_already_early_stopped\tstart_epoch={start_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
        );
    } else {
        for epoch in start_epoch..=max_epochs {
            ...
            println!("v0270_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
            ...
            for local_step in 0..steps_per_epoch {
                ...
                if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                    println!(
                        "v0270_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={total_value:.6}\trelation={relation_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                        local_step + 1,
            ...
            if improved {
                ...
                println!(
                    "v0270_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}"
                );
            } else {
                stale_epochs += 1;
            }
            ...
            println!(
                "v0270_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}"
            );
            if stale_epochs >= patience {
                stopped_early = true;
                println!(
                    "v0270_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}"
                );
                break;
            }
        }
    ...
    println!("historical_validation_consumed\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0270_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");
    println!("v0270_finalize_required\tYES");
...
fn finite_ratio(value: Option<f64>, baseline: Option<f64>, label: &str) -> Result<f64> {
    match (value, baseline) {
        ...
        _ => anyhow::bail!("v0.27 dev objective is missing {label} metric"),
    }
}
...
fn finite_ratio_value(value: f64, baseline: f64, label: &str) -> Result<f64> {
    if !value.is_finite() || !baseline.is_finite() || baseline.abs() <= 1.0e-12 {
        anyhow::bail!(
            "v0.27 dev objective has invalid {label} ratio: value={value}, baseline={baseline}"
        );
    }
    Ok(value / baseline)
}
...
fn copy_checkpoint_dir(source: &Path, destination: &Path) -> Result<()> {
    ...
    for name in [
        ...
        fs::copy(source.join(name), destination.join(name)).with_context(|| {
            format!("failed to copy v0.27 checkpoint file {name} from {source:?}")
        })?;
    }
    Ok(())
}
...
fn forward_loss(
    model: &PeptideFoundationMultimodalV0270Model,
    ...
    let fragment_context =
        FoundationFragmentContextBatchV0270::from_records(records, model.forward_config(), device)?;
    ...
    let second_projection = model
        .forward()
        .peptide_projection_t(&views.second.input, true)?;
...
fn cross_modal_relation_metrics(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.27 relation objective requires at least two records");
    }
    ...
    let loss = foundation_multimodal_relation_margin_loss_v0270(
        &positive_score,
        &negative_score,
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0270,
    )?;
    ...
    if positive.len() != negative.len() || positive.is_empty() {
        anyhow::bail!("v0.27 relation diagnostics require equal non-empty score batches");
    }
...
fn mass_near_negative_records(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.27 mass-near negatives require at least two records");
    }
    let masses = records
        .iter()
        .map(|record| {
...
fn evaluate_properties(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let fragment_context = FoundationFragmentContextBatchV0270::from_records(
            &owned,
            model.forward_config(),
            device,
        )?;
        let fragment_channel_mask = fragment_context.channel_mask()?;
        let output = model.forward().forward_v0270_t(
            &batch.input,
            &batch.context,
            &fragment_context,
            false,
...
fn load_v0261_unchanged_variables(
    varmap: &VarMap,
    checkpoint: &Path,
    device: &Device,
) -> Result<V027WarmStartReport> {
    let tensors = candle_core::safetensors::load(checkpoint, device)
        .with_context(|| format!("failed to load frozen v0.26.1 checkpoint {checkpoint:?}"))?;
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.27 VarMap lock poisoned during warm start"))?;
    let mut loaded_variables = 0usize;
    let mut fresh_variables = 0usize;
    let mut missing_reused = Vec::new();

    for (name, variable) in data.iter() {
        if name.starts_with("rt_specialist.") || name.starts_with("fragment_decoder.") {
            fresh_variables += 1;
            continue;
        }
        match tensors.get(name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.27 warm-start shape mismatch for '{name}': parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing_reused.is_empty() {
        anyhow::bail!(
            "frozen v0.26.1 checkpoint is missing unchanged v0.27 variables: {}",
            missing_reused.join(", ")
        );
    }
    ...
    if loaded_variables == 0 || fresh_variables == 0 {
        anyhow::bail!(
            "v0.27 selective warm start is nonfunctional: loaded={loaded_variables} fresh={fresh_variables}"
        );
    }
    Ok(V027WarmStartReport {
        loaded_variables,
        fresh_variables,
...
fn validate_v0270_training_checkpoint_metadata(
    label: &str,
    metadata: &UnifiedPilotMetadata,
    expected_corpus_fingerprint: &str,
    expected_benchmark_fingerprint: &str,
    expected_config: &PeptideFoundationMultimodalV0270Config,
    batch_size: usize,
    seed: u64,
    learning_rate: f64,
    max_total_steps: usize,
) -> Result<()> {
    if metadata.version != 270 {
        anyhow::bail!(
            "v0.27 resume {label} checkpoint has metadata version {}, expected 270",
            metadata.version
        );
    }
    if metadata.objective != "v0270_rt_specialist_contextual_fragment_tokens_openptm32" {
        anyhow::bail!(
            "v0.27 resume {label} checkpoint objective mismatch: {}",
            metadata.objective
        );
    }
    if metadata.corpus_fingerprint != expected_corpus_fingerprint {
        anyhow::bail!(
            "v0.27 resume {label} corpus fingerprint mismatch: checkpoint={} current={expected_corpus_fingerprint}",
            metadata.corpus_fingerprint
        );
    }
    if metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
        anyhow::bail!(
            "v0.27 resume {label} benchmark fingerprint mismatch: checkpoint={} current={expected_benchmark_fingerprint}",
            metadata.benchmark_manifest_fingerprint
        );
    }
    if metadata.v0270_config != *expected_config {
        anyhow::bail!("v0.27 resume {label} architecture config mismatch");
    }
    if metadata.batch_size != batch_size {
        anyhow::bail!(
            "v0.27 resume {label} batch_size mismatch: checkpoint={} requested={batch_size}",
            metadata.batch_size
        );
    }
    if metadata.seed != seed {
        anyhow::bail!(
            "v0.27 resume {label} seed mismatch: checkpoint={} requested={seed}",
            metadata.seed
        );
    }
    ...
    if (metadata.learning_rate - learning_rate).abs() > f64::EPSILON * 32.0 * lr_scale {
        anyhow::bail!(
            "v0.27 resume {label} learning_rate mismatch: checkpoint={} requested={learning_rate}",
            metadata.learning_rate
        );
    }
    if metadata.train_steps != max_total_steps {
        anyhow::bail!(
            "v0.27 resume {label} train-step budget mismatch: checkpoint={} requested={max_total_steps}",
            metadata.train_steps
        );
    }
    if metadata.completed_steps > metadata.train_steps {
        anyhow::bail!(
            "v0.27 resume {label} checkpoint completed_steps={} exceeds train_steps={}",
...
fn read_unified_pilot_metadata(checkpoint: &Path) -> Result<UnifiedPilotMetadata> {
    ...
    serde_yaml::from_str(
        &fs::read_to_string(&metadata_path)
            .with_context(|| format!("failed to read v0.27 metadata {metadata_path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.27 metadata {metadata_path:?}"))
}
...
fn feasible_validation_batches(
    ...
    if batch_size == 0 || requested_batches == 0 {
        anyhow::bail!("v0.27 {label} validation requires positive batch_size/requested_batches");
    }
    let max_batches = requested_batches.min(validation_indices.len() / batch_size);
    if max_batches == 0 {
        anyhow::bail!(
            "v0.27 {label} has only {} records, fewer than batch_size={batch_size}",
    ...
    for &index in validation_indices {
        let source = provenance.get(index).ok_or_else(|| {
            anyhow::anyhow!("v0.27 {label} provenance index {index} is out of bounds")
        })?;
    ...
    for source in available.keys() {
        if !config.validation_source_weights.contains_key(source) {
            anyhow::bail!(
                "v0.27 {label} validation weights are missing represented source '{source}'"
            );
        }
    }
    for source in config.validation_source_weights.keys() {
        if !available.contains_key(source) {
            anyhow::bail!("v0.27 {label} validation weight refers to absent source '{source}'");
        }
    }

    let total_weight: f64 = config.validation_source_weights.values().copied().sum();
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.27 {label} validation source weights have no positive finite mass");
    }

    for batches in (1..=max_batches).rev() {
        let target_records = batches.saturating_mul(batch_size);
        let quotas = weighted_quotas_v0270(
            target_records,
    ...
    anyhow::bail!(
        "v0.27 {label} cannot satisfy validation source weights for even one full batch without replacement"
    )
}

fn weighted_quotas_v0270(
