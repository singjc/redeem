...
use redeem_properties::foundation::{
    ...
    PeptideFoundationMultimodalV0260Model, RetentionTimeObjective,
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0260,
};
...
const VERSION: &str = "v0.26.1-alphapeptdeep-comparison-export";
...
fn main() -> Result<()> {
    ...
    if !(3..=6).contains(&args.len()) {
        anyhow::bail!(
            "usage: foundation_export_alphapeptdeep_comparison_v0261 RUN.yaml V0261_CHECKPOINT OUTPUT_DIR [validation_per_task=all_if_omitted] [rt_train_calibration=256] [seed=20260916]"
        );
    }
    ...
    if metadata.version != 261 {
        anyhow::bail!(
            "expected v0.26.1 checkpoint metadata version 261, observed {}",
            metadata.version
        );
    }
    ...
    let model = PeptideFoundationMultimodalV0260Model::new(
        ...
        metadata.property_adapter_hidden,
        vb,
    )?;
    varmap
        .load(checkpoint.join("model.safetensors"))
        .with_context(|| format!("load v0.26.1 multimodal checkpoint {checkpoint:?}"))?;
