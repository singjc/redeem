//! v0.30 shared fragment-aware shared-representation continuation warm-started from frozen v0.27.
//!
...
//!
//! Every v0.27 namespace is restored exactly. Only one training-only shared-residue
//! cleavage-local fragment auxiliary is new; RT/CCS/MS2 heads and all inverse
//! objectives remain unchanged. Historical VALIDATION and TEST stay outside
...
use redeem_properties::foundation::{
    ...
    foundation_fragment_representation_aux_loss_v0300, foundation_ms2_loss,
    foundation_multimodal_ms2_loss_v0300, foundation_multimodal_relation_margin_loss_v0300,
    ...
    FoundationDiffusionConfig, FoundationDiffusionVocabulary, FoundationFragmentContextBatchV0300,
    ...
    PeptideFoundationMultimodalV0270Config, PeptideFoundationMultimodalV0300Config,
    PeptideFoundationMultimodalV0300Model, PeptidoformInput, PrecursorContextBatch,
    ...
    FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0300, FOUNDATION_MS2_SOFTPLUS_BETA_V0138,
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0300, FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0300,
    FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0300, FOUNDATION_OPEN_PTM_MASS_SCALE_DA,
...
struct UnifiedPilotMetadata {
    ...
    forward_config: redeem_properties::foundation::FoundationConfig,
    inverse_config: FoundationDiffusionConfig,
    v0300_config: PeptideFoundationMultimodalV0300Config,
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0300 RUN_V0260.yaml OUTPUT_DIR PARENT_V0270_CHECKPOINT [max_epochs=12] [batch_size=32] [patience=4] [min_delta=0.001] [seed=20260930] [learning_rate=2.5e-5] [mode=train|resume|finalize]"
        );
    }
    ...
    let min_delta = parse_or(&args, 7, 0.001f64)?;
    let seed = parse_or(&args, 8, 20_260_930u64)?;
    ...
    let (resume_training, finalize_only) = match run_mode {
        ...
        "finalize" => (false, true),
        other => anyhow::bail!(
            "unsupported v0.30 run mode {other:?}; expected train, resume, or finalize"
        ),
    };
    ...
    if finalize_only || resume_training {
        if !output_root.is_dir() {
            anyhow::bail!(
                "v0.30 {run_mode} mode requires an existing model output directory: {:?}",
                output_root
            );
        }
        ...
        for checkpoint in required_checkpoints {
            for name in [
                ...
                if !path.is_file() {
                    anyhow::bail!("v0.30 {run_mode} mode is missing checkpoint file: {path:?}");
                }
            }
        }
    } else if output_root.exists() {
        anyhow::bail!("v0.30 output directory already exists: {:?}", output_root);
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.30 requires a CUDA device")?;
    ...
    if parent_metadata.version != 270 {
        anyhow::bail!(
            "v0.30 requires the frozen v0.27 parent metadata version 270, observed {}",
            parent_metadata.version
        );
    }
    if parent_metadata.objective != "v0270_rt_specialist_contextual_fragment_tokens_openptm32" {
        anyhow::bail!(
            "v0.30 requires the frozen v0.27 objective, observed {}",
            parent_metadata.objective
        );
    }
    // v0.30 changes only the property view of the accepted v0.27 shared
    ...
    if parent_metadata.v0270_config != expected_v0270 {
        anyhow::bail!("v0.30 parent v0.27 architecture config does not match its base configs");
    }
    let v0300_config =
        PeptideFoundationMultimodalV0300Config::fixed(parent_metadata.v0270_config.clone())?;

    // The v0.30 prepare step excludes peptides that cannot be represented by
    ...
    if let Some((record_index, sequence, sequence_len)) = first_overlength {
        anyhow::bail!(
            "v0.30 prepared benchmark contains {prepared_overlength_records} peptide(s) longer than architecture max_sequence_len={}; first record_index={record_index} length={sequence_len} sequence={sequence}",
    ...
    let model = PeptideFoundationMultimodalV0300Model::new(v0300_config.clone(), vb)?;

    // Restore every v0.27 namespace exactly. Only fragment_representation_aux.* is absent
    // from the parent. The auxiliary is training-only, so accepted v0.27 property
    // predictions reproduce the frozen parent exactly at step 0.
    ...
    let warm_start_description = format!(
        "v0270_all_namespaces_plus_fresh_fragment_representation_aux_from={}",
    ...
    println!("v0300_version\tv0.30-fragment-aware-shared-residue-aux");
    println!("objective\tv0300_fragment_aware_shared_residue_aux_v027_heads_openptm32");
    println!("architecture\t{}", FOUNDATION_MULTIMODAL_ARCHITECTURE_V0300);
    ...
    println!("random_initialization\tfragment_representation_aux_only");
    println!(
    ...
    println!(
        "relation_margin\t{}",
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0300
    );
    println!(
        "property_adapter_hidden\t{}",
        FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0300
    );
    ...
    println!(
        ...
        FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0300
    );
    println!("fragment_representation_aux_context\tcharge_presence_nce_presence");
    println!("accepted_property_heads\tpreserved_exactly_from_v0270");
    ...
    let metadata = |completed_steps| UnifiedPilotMetadata {
        version: 300,
        objective: "v0300_fragment_aware_shared_residue_aux_v027_heads_openptm32".into(),
        ...
        relation_margin: FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0300,
        property_adapter_hidden: FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0300,
        ...
        v0300_config: v0300_config.clone(),
    ...
    if finalize_only {
        ...
        if best_metadata.version != 300 {
            anyhow::bail!(
                "v0.30 finalize mode expected checkpoint metadata version 300, observed {}",
                best_metadata.version
            );
        }
        if best_metadata.completed_steps == 0 {
            anyhow::bail!(
                "v0.30 has no DEV improvement over frozen v0.27; refusing to consume HOLDOUT/finalize baseline"
            );
        }
        if best_metadata.corpus_fingerprint != expected_corpus_fingerprint {
            anyhow::bail!(
                "v0.30 finalize corpus fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
            anyhow::bail!(
                "v0.30 finalize benchmark fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.batch_size != batch_size {
            anyhow::bail!(
                "v0.30 finalize batch_size mismatch: checkpoint={} requested={batch_size}",
                best_metadata.batch_size
            );
        }
        if best_metadata.seed != seed {
            anyhow::bail!(
                "v0.30 finalize seed mismatch: checkpoint={} requested={seed}",
                best_metadata.seed
            );
        }
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore best v0.30 model {best_model:?}"))?;
        ...
        println!("v0300_finalize_only\ttrue");
        println!("v0300_finalize_source_checkpoint\t{}", best_dir.display());
        println!("v0300_finalize_best_step\t{best_step}");
        println!(
            "v0300_finalize_best_epoch\t{}",
            best_epoch
        ...
        println!("v0300_finalize_complete\tbest_step={best_step}");
    ...
    let (
        ...
        for (label, checkpoint_metadata) in [
            ...
            validate_v0300_training_checkpoint_metadata(
                label,
                checkpoint_metadata,
                &expected_corpus_fingerprint,
                &expected_benchmark_fingerprint,
        ...
        if initial_metadata.completed_steps != 0 {
            anyhow::bail!(
                "v0.30 resume expected initial checkpoint at step 0, observed {}",
        ...
        {
            anyhow::bail!(
                "v0.30 resume requires latest checkpoint at a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                latest_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps % steps_per_epoch != 0 {
            anyhow::bail!(
                "v0.30 resume requires best checkpoint at baseline or a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                best_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps > latest_metadata.completed_steps {
            anyhow::bail!(
                "v0.30 resume best checkpoint step {} is newer than latest step {}",
                best_metadata.completed_steps,
                latest_metadata.completed_steps
            );
        }
        ...
        varmap
            .load(&initial_model)
            .with_context(|| format!("failed to restore v0.30 initial model {initial_model:?}"))?;
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore v0.30 best model {best_model:?}"))?;
        ...
        varmap
            .load(&latest_model)
            .with_context(|| format!("failed to restore v0.30 latest model {latest_model:?}"))?;
        let latest_optimizer = latest_dir.join("optimizer.safetensors");
        optimizer
            .load_safetensors(&latest_optimizer)
            .with_context(|| {
                format!("failed to restore v0.30 optimizer state {latest_optimizer:?}")
            })?;
        ...
        if stale_epochs >= patience {
            println!(
                "v0300_resume_already_early_stopped\tcompleted_epoch={completed_epoch}\tbest_epoch={best_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
            );
        }
        println!(
            "v0300_resume\tlatest_epoch={completed_epoch}\tlatest_step={global_step}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstale_epochs={stale_epochs}"
        );
        (
            initial_metrics,
            global_step,
        ...
        // v0.30 is an identity-preserving continuation at the accepted output
    ...
    if start_epoch > max_epochs {
        println!(
            "v0300_training_budget_complete\tstart_epoch={start_epoch}\tmax_epochs={max_epochs}\tlatest_step={global_step}"
        );
    } else if stopped_early {
        println!(
            "v0300_training_already_early_stopped\tstart_epoch={start_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
        );
    } else {
        for epoch in start_epoch..=max_epochs {
            ...
            println!("v0300_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
            ...
            for local_step in 0..steps_per_epoch {
                ...
                if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                    println!(
                        "v0300_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={total_value:.6}\trelation={relation_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                        local_step + 1,
            ...
            if improved {
                ...
                println!(
                    "v0300_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}"
                );
            } else {
                stale_epochs += 1;
            }
            ...
            println!(
                "v0300_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}"
            );
            if stale_epochs >= patience {
                stopped_early = true;
                println!(
                    "v0300_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}"
                );
                break;
            }
        }
    ...
    println!("historical_validation_consumed\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0300_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");
    if best_step == 0 {
        println!("v0300_no_dev_improvement_over_parent\tYES");
        println!("v0300_finalize_required\tNO");
    } else {
        println!("v0300_no_dev_improvement_over_parent\tNO");
        println!("v0300_finalize_required\tYES");
    }
...
fn finite_ratio(value: Option<f64>, baseline: Option<f64>, label: &str) -> Result<f64> {
    match (value, baseline) {
        ...
        _ => anyhow::bail!("v0.30 dev objective is missing {label} metric"),
    }
}
...
fn finite_ratio_value(value: f64, baseline: f64, label: &str) -> Result<f64> {
    if !value.is_finite() || !baseline.is_finite() || baseline.abs() <= 1.0e-12 {
        anyhow::bail!(
            "v0.30 dev objective has invalid {label} ratio: value={value}, baseline={baseline}"
        );
    }
    Ok(value / baseline)
}
...
fn copy_checkpoint_dir(source: &Path, destination: &Path) -> Result<()> {
    ...
    for name in [
        ...
        fs::copy(source.join(name), destination.join(name)).with_context(|| {
            format!("failed to copy v0.30 checkpoint file {name} from {source:?}")
        })?;
    }
    Ok(())
}
...
fn forward_loss(
    model: &PeptideFoundationMultimodalV0300Model,
    ...
    let fragment_context =
        FoundationFragmentContextBatchV0300::from_records(records, model.forward_config(), device)?;
    let fragment_channel_mask = fragment_context.channel_mask()?;
    let first = model.forward_v0300_t_with_shared_gradient_scales(
        &views.first.input,
        &views.first.context,
        &fragment_context,
        true,
    ...
    let factorized = foundation_multimodal_ms2_loss_v0300(
        &first,
    ...
    if let Some(auxiliary) = foundation_fragment_representation_aux_loss_v0300(
        &first,
        views.first.targets.ms2.as_ref(),
        contextual_ms2_mask.as_ref(),
        ms2_loss,
        ...
        total = (total
            + auxiliary
                .total
                .affine(FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0300, 0.0)?)?;
    }
...
fn cross_modal_relation_metrics(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.30 relation objective requires at least two records");
    }
    ...
    let positive_foundation = model.encoder().forward_t(&positive_batch.input, train)?;
    let negative_foundation = model.encoder().forward_t(&negative_batch.input, train)?;
    ...
    let loss = foundation_multimodal_relation_margin_loss_v0300(
        &positive_score,
        &negative_score,
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0300,
    )?;
    ...
    if positive.len() != negative.len() || positive.is_empty() {
        anyhow::bail!("v0.30 relation diagnostics require equal non-empty score batches");
    }
...
fn mass_near_negative_records(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.30 mass-near negatives require at least two records");
    }
    let masses = records
        .iter()
        .map(|record| {
...
fn evaluate_properties(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let fragment_context = FoundationFragmentContextBatchV0300::from_records(
            &owned,
        ...
        let fragment_channel_mask = fragment_context.channel_mask()?;
        let output =
            model.forward_v0300_t(&batch.input, &batch.context, &fragment_context, false)?;
...
fn load_v0270_unchanged_variables(
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.30 VarMap lock poisoned during warm start"))?;
    let mut loaded_variables = 0usize;
    let mut fresh_variables = 0usize;
    let mut missing_reused = Vec::new();

    for (name, variable) in data.iter() {
        if name.starts_with("fragment_representation_aux.") {
            fresh_variables += 1;
            continue;
        }
        match tensors.get(name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.30 warm-start shape mismatch for '{name}': parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing_reused.is_empty() {
        anyhow::bail!(
            "frozen v0.27 checkpoint is missing required v0.30 base variables: {}",
            missing_reused.join(", ")
        );
    }
    ...
    if loaded_variables == 0 || fresh_variables == 0 {
        anyhow::bail!(
            "v0.30 selective warm start is nonfunctional: loaded={loaded_variables} fresh={fresh_variables}"
        );
    }
    Ok(V030WarmStartReport {
        loaded_variables,
        fresh_variables,
...
fn validate_v0300_training_checkpoint_metadata(
    label: &str,
    metadata: &UnifiedPilotMetadata,
    expected_corpus_fingerprint: &str,
    expected_benchmark_fingerprint: &str,
    expected_config: &PeptideFoundationMultimodalV0300Config,
    batch_size: usize,
    seed: u64,
    learning_rate: f64,
    max_total_steps: usize,
) -> Result<()> {
    if metadata.version != 300 {
        anyhow::bail!(
            "v0.30 resume {label} checkpoint has metadata version {}, expected 300",
            metadata.version
        );
    }
    if metadata.objective != "v0300_fragment_aware_shared_residue_aux_v027_heads_openptm32" {
        anyhow::bail!(
            "v0.30 resume {label} checkpoint objective mismatch: {}",
            metadata.objective
        );
    }
    if metadata.corpus_fingerprint != expected_corpus_fingerprint {
        anyhow::bail!(
            "v0.30 resume {label} corpus fingerprint mismatch: checkpoint={} current={expected_corpus_fingerprint}",
            metadata.corpus_fingerprint
        );
    }
    if metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
        anyhow::bail!(
            "v0.30 resume {label} benchmark fingerprint mismatch: checkpoint={} current={expected_benchmark_fingerprint}",
            metadata.benchmark_manifest_fingerprint
        );
    }
    if metadata.v0300_config != *expected_config {
        anyhow::bail!("v0.30 resume {label} architecture config mismatch");
    }
    if metadata.batch_size != batch_size {
        anyhow::bail!(
            "v0.30 resume {label} batch_size mismatch: checkpoint={} requested={batch_size}",
            metadata.batch_size
        );
    }
    if metadata.seed != seed {
        anyhow::bail!(
            "v0.30 resume {label} seed mismatch: checkpoint={} requested={seed}",
            metadata.seed
        );
    }
    ...
    if (metadata.learning_rate - learning_rate).abs() > f64::EPSILON * 32.0 * lr_scale {
        anyhow::bail!(
            "v0.30 resume {label} learning_rate mismatch: checkpoint={} requested={learning_rate}",
            metadata.learning_rate
        );
    }
    if metadata.train_steps != max_total_steps {
        anyhow::bail!(
            "v0.30 resume {label} train-step budget mismatch: checkpoint={} requested={max_total_steps}",
            metadata.train_steps
        );
    }
    if metadata.completed_steps > metadata.train_steps {
        anyhow::bail!(
            "v0.30 resume {label} checkpoint completed_steps={} exceeds train_steps={}",
...
fn read_unified_pilot_metadata(checkpoint: &Path) -> Result<UnifiedPilotMetadata> {
    ...
    serde_yaml::from_str(
        &fs::read_to_string(&metadata_path)
            .with_context(|| format!("failed to read v0.30 metadata {metadata_path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.30 metadata {metadata_path:?}"))
}
...
fn feasible_validation_batches(
    ...
    if batch_size == 0 || requested_batches == 0 {
        anyhow::bail!("v0.30 {label} validation requires positive batch_size/requested_batches");
    }
    let max_batches = requested_batches.min(validation_indices.len() / batch_size);
    if max_batches == 0 {
        anyhow::bail!(
            "v0.30 {label} has only {} records, fewer than batch_size={batch_size}",
    ...
    for &index in validation_indices {
        let source = provenance.get(index).ok_or_else(|| {
            anyhow::anyhow!("v0.30 {label} provenance index {index} is out of bounds")
        })?;
    ...
    for source in available.keys() {
        if !config.validation_source_weights.contains_key(source) {
            anyhow::bail!(
                "v0.30 {label} validation weights are missing represented source '{source}'"
            );
        }
    }
    for source in config.validation_source_weights.keys() {
        if !available.contains_key(source) {
            anyhow::bail!("v0.30 {label} validation weight refers to absent source '{source}'");
        }
    }

    let total_weight: f64 = config.validation_source_weights.values().copied().sum();
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.30 {label} validation source weights have no positive finite mass");
    }

    for batches in (1..=max_batches).rev() {
        let target_records = batches.saturating_mul(batch_size);
        let quotas = weighted_quotas_v0300(
            target_records,
    ...
    anyhow::bail!(
        "v0.30 {label} cannot satisfy validation source weights for even one full batch without replacement"
    )
}

fn weighted_quotas_v0300(
