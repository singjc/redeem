//! v0.37 robust-consensus reliability-weighted CCS refinement from frozen v0.35.
//!
...
use redeem_properties::foundation::{
    ...
    PeptideFoundationMultimodalV0350Config, PeptideFoundationMultimodalV0360Config,
    PeptideFoundationMultimodalV0360Model, RetentionTimeObjective,
...
const V037_VERSION: u32 = 370;
const V037_OBJECTIVE: &str =
    "v0370_frozen_v0350_train_consensus_reliability_weighted_ccs_refinement";
const V037_ARCHITECTURE: &str = "v0.37-frozen-v0350-consensus-reliability-weighted-ccs-specialist";
const V037_MAX_STEPS_PER_EPOCH: usize = 1_536;
const V037_MAX_DEV_BATCHES: usize = 32;
const V037_MAX_HOLDOUT_BATCHES: usize = 32;
const V037_LOSS_CALIBRATION_RECORDS: usize = 4_096;
const V037_CCS_MSE_WEIGHT: f64 = 0.25;
const V037_CCS_ROBUST_WEIGHT: f64 = 1.0;
const V037_MIN_LOSS_SCALE: f64 = 1.0e-3;
const V037_MAX_GRADIENT_NORM: f64 = 1.0;
const V037_MATERIAL_RAW_CCS_RATIO: f64 = 0.93;
const V037_MATERIAL_CONSENSUS_CCS_RATIO: f64 = 0.90;
const V037_RAW_OBJECTIVE_WEIGHT: f64 = 0.70;
const V037_CONSENSUS_OBJECTIVE_WEIGHT: f64 = 0.30;
const V037_RT_INVARIANCE_TOLERANCE: f64 = 5.0e-5;
const V037_MS2_INVARIANCE_TOLERANCE: f64 = 5.0e-5;
const V037_SOURCE_SHRINKAGE: f64 = 256.0;
const V037_MIN_SOURCE_SHARED_IDENTITIES: usize = 20;
const V037_RELIABILITY_FLOOR: f64 = 0.35;
const V037_RELIABILITY_CEILING: f64 = 1.25;
const V037_SINGLETON_WEIGHT_SCALE: f64 = 0.65;
const V037_CONSENSUS_DISPERSION_SCALE: f64 = 5.0;
...
struct V037Metadata {
    ...
    seed: u64,
    learning_rate: f64,
    ccs_loss_scale_model_space: f64,
    completed_steps: usize,
...
#[derive(Debug, Clone)]
struct ConsensusSupervision {
    examples: Vec<ConsensusExample>,
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0370 RUN_V0260.yaml OUTPUT_DIR PARENT_V0350_CHECKPOINT [max_epochs=8] [batch_size=128] [patience=3] [min_delta=0.002] [seed=20261037] [learning_rate=6e-5] [mode=train|finalize]"
        );
    }
    ...
    let seed = parse_or(&args, 8, 20_261_037u64)?;
    let learning_rate = parse_or(&args, 9, 6.0e-5f64)?;
    let run_mode = args.get(10).map(String::as_str).unwrap_or("train");
    let finalize_only = match run_mode {
        "train" => false,
        "finalize" => true,
        other => anyhow::bail!("unsupported v0.37 run mode {other:?}; expected train or finalize"),
    };

    if max_epochs == 0 || batch_size < 2 || patience == 0 {
        anyhow::bail!("v0.37 requires max_epochs>0, batch_size>=2, and patience>0");
    }
    if !(min_delta > 0.0 && min_delta.is_finite()) {
        anyhow::bail!("v0.37 min_delta must be positive and finite");
    }
    if !(learning_rate > 0.0 && learning_rate.is_finite()) {
        anyhow::bail!("v0.37 learning_rate must be positive and finite");
    }
        ...
        for name in ["initial", "best"] {
            ...
            for file in [
                ...
                if !directory.join(file).is_file() {
                    anyhow::bail!("v0.37 finalize is missing {:?}", directory.join(file));
                }
            }
        }
        if output_root.join("final").exists() {
            anyhow::bail!(
                "v0.37 final checkpoint already exists; HOLDOUT must not be consumed twice"
            );
        }
    } else if output_root.exists() {
        anyhow::bail!(
            "v0.37 TRAIN output directory already exists: {:?}",
            output_root
        );
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.37 requires a CUDA device")?;
    ...
    let holdout_indices: Vec<usize> = benchmark
        ...
        .map(|entry| entry.record_index)
        .collect();
    let ccs_train_indices = finite_ccs_indices(&corpus.records, &train_indices);
    let ccs_dev_indices = finite_ccs_indices(&corpus.records, &dev_indices);
    if ccs_train_indices.len() < batch_size || ccs_dev_indices.len() < batch_size {
        anyhow::bail!(
            "v0.37 requires at least one batch of CCS labels in TRAIN and DEV; observed train={} dev={} batch_size={batch_size}",
            ccs_train_indices.len(),
            ccs_dev_indices.len()
        );
    }
    ...
    if parent_metadata.version != 350 {
        anyhow::bail!(
            "v0.37 requires v0.35 metadata version 350, observed {}",
    ...
    {
        anyhow::bail!(
            "v0.37 requires the accepted v0.35 objective, observed {}",
            parent_metadata.objective
        );
    }
    if parent_metadata.completed_steps == 0 {
        anyhow::bail!("v0.37 refuses an unselected v0.35 baseline checkpoint");
    }
    ...
    if parent_metadata.corpus_fingerprint != current_corpus_fingerprint {
        anyhow::bail!("v0.37 corpus fingerprint differs from frozen v0.35 parent");
    }
    if parent_metadata.benchmark_manifest_fingerprint != current_benchmark_fingerprint {
        anyhow::bail!("v0.37 benchmark fingerprint differs from frozen v0.35 parent");
    }
        ...
        .chain(&holdout_indices)
    {
        ...
        if length > forward_config.max_sequence_len {
            anyhow::bail!(
                "v0.37 record {index} length {length} exceeds max_sequence_len={}",
    ...
    let train_supervision =
        build_train_consensus_supervision(&corpus.records, &corpus.provenance, &ccs_train_indices)?;
    if train_supervision.examples.len() < batch_size {
        anyhow::bail!("v0.37 consensus TRAIN has fewer examples than batch_size");
    }
    ...
    let calibration_count = V037_LOSS_CALIBRATION_RECORDS
    ...
    let calibration_order = deterministic_example_order(
        ...
        calibration_count,
        0,
        seed ^ 0x3700_4c4f_5353_4343,
    );
    let calibration_examples = calibration_order
        .iter()
        .map(|&index| train_supervision.examples[index].clone())
        .collect::<Vec<_>>();
    let ccs_loss_scale_model_space = calibrate_consensus_residual_scale(
        &model,
        &clean_collator,
        &corpus.records,
    ...
    let optimizer_prefixes = ["ccs_specialist_v0360."];
    ...
    if !finalize_only {
        ...
        write_supervision_summary(&output_root, &train_supervision)?;
    }

    println!("v0370_version\tv0.37-consensus-reliability-weighted-ccs");
    println!("objective\t{V037_OBJECTIVE}");
    println!("architecture\t{V037_ARCHITECTURE}");
    ...
    println!("raw_train_ccs_records\t{}", train_supervision.raw_records);
    println!(
    ...
    println!(
        "ccs_loss_calibration_records\t{}",
        calibration_examples.len()
    );
    println!("ccs_loss_scale_model_space\t{ccs_loss_scale_model_space:.8}");
    println!("ccs_loss_objective\tweighted_0.25_scaled_mse_plus_1.0_scaled_pseudo_huber");
    println!("lr_schedule\twarmup_cosine_500_to_0.10");
    println!("optimizer_scope\tccs_specialist_v0360_only");
    println!("optimizer_variable_count\t{}", optimizer.variable_count());
    println!("v0350_update_policy\tfrozen_detached_anchor");
    println!("rt_update_policy\texact_frozen_zero_residual");
    ...
    println!("training_target_policy\ttrain_only_source_affine_then_robust_reliability_weighted_consensus");
    println!("overlapping_view_policy\tdeduplicated_by_exact_peptidoform_charge");
    println!(
        "material_dev_gate\traw_ccs_ratio<={V037_MATERIAL_RAW_CCS_RATIO:.2}_and_consensus_ccs_ratio<={V037_MATERIAL_CONSENSUS_CCS_RATIO:.2}_and_rt_ms2_invariant"
    );
    println!("historical_validation_reused_for_v0370_selection\tNO");
    ...
    if finalize_only {
        ...
        let best_metadata = read_v037_metadata(&best_dir)?;
        validate_metadata(
            &best_metadata,
        ...
        if best_metadata.completed_steps == 0 {
            anyhow::bail!("v0.37 has no DEV improvement; refusing to consume HOLDOUT");
        }
        ...
        let best_raw_dev = evaluate_ccs_indices(
            &model,
            &clean_collator,
            &corpus.records,
            &ccs_dev_indices,
        ...
        let best_consensus_dev = evaluate_ccs_consensus(
            &model,
            &clean_collator,
            &corpus.records,
            &dev_consensus,
        ...
        println!("v0370_finalize_dev_objective\t{objective:.8}");
        println!("v0370_finalize_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0370_finalize_consensus_ccs_ratio\t{consensus_ratio:.8}");
        if raw_ratio > V037_MATERIAL_RAW_CCS_RATIO
            || consensus_ratio > V037_MATERIAL_CONSENSUS_CCS_RATIO
        {
            anyhow::bail!(
                "v0.37 DEV result does not satisfy the fixed materiality gate; refusing HOLDOUT"
            );
        }

        println!("v0370_finalize_only\ttrue");
        println!(
            "v0370_finalize_best_step\t{}",
            best_metadata.completed_steps
        );
        let holdout_ccs_indices = finite_ccs_indices(&corpus.records, &holdout_indices);
        ...
        let holdout_raw_ccs = evaluate_ccs_indices(
            &model,
            &clean_collator,
            &corpus.records,
            &holdout_ccs_indices,
        ...
        let holdout_consensus_ccs = evaluate_ccs_consensus(
            &model,
            &clean_collator,
            &corpus.records,
            &holdout_consensus,
        ...
        println!(
            "v0370_holdout_raw_ccs\trecords={}\tmae={holdout_raw_ccs:.8}",
            holdout_ccs_indices.len()
        );
        println!(
            "v0370_holdout_consensus_ccs\tidentities={}\tmae={holdout_consensus_ccs:.8}",
        ...
        println!(
            "v0370_finalize_complete\tbest_step={}",
            best_metadata.completed_steps
        );
    ...
    let initial_raw_dev_ccs = evaluate_ccs_indices(
        &model,
        &clean_collator,
        &corpus.records,
        &ccs_dev_indices,
    ...
    let initial_consensus_dev_ccs = evaluate_ccs_consensus(
        &model,
        &clean_collator,
        &corpus.records,
        &dev_consensus,
    ...
    let metadata = |completed_steps| V037Metadata {
        version: V037_VERSION,
        objective: V037_OBJECTIVE.into(),
        architecture: V037_ARCHITECTURE.into(),
    ...
    for epoch in 1..=max_epochs {
        ...
        let order = deterministic_example_order(
            ...
            needed,
            epoch as u64,
            seed ^ 0x3700_7a11_2233_4455,
        );
        println!(
            "v0370_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}\texamples={needed}"
        );
        ...
        for local_step in 0..steps_per_epoch {
            ...
            let selected = order[offset..offset + batch_size]
                ...
                .collect::<Vec<_>>();
            let loss = consensus_scalar_loss(
                &model,
                &clean_collator,
                &corpus.records,
                &selected,
            ...
            let update = optimizer.backward_step(&loss, Some(V037_MAX_GRADIENT_NORM))?;
            if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                println!(
                    "v0370_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={loss_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                    local_step + 1,
        ...
        let raw_dev_ccs = evaluate_ccs_indices(
            &model,
            &clean_collator,
            &corpus.records,
            &ccs_dev_indices,
        ...
        let consensus_dev_ccs = evaluate_ccs_consensus(
            &model,
            &clean_collator,
            &corpus.records,
            &dev_consensus,
        ...
        if improved {
            ...
            println!("v0370_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}\traw_ccs_ratio={raw_ratio:.8}\tconsensus_ccs_ratio={consensus_ratio:.8}");
        } else {
            stale_epochs += 1;
        }
        println!("v0370_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}");
        if stale_epochs >= patience {
            stopped_early = true;
            println!("v0370_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}");
            break;
        }
    }
    ...
    println!("historical_validation_reused_for_v0370_selection\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0370_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");

    if best_step == 0 {
        println!("v0370_material_dev_gain\tNO");
        println!("v0370_finalize_required\tNO");
        println!("v0370_rethink_required\tYES");
    } else {
        ...
        let material = raw_ratio <= V037_MATERIAL_RAW_CCS_RATIO
            && consensus_ratio <= V037_MATERIAL_CONSENSUS_CCS_RATIO;
        println!("v0370_best_raw_dev_ccs_mae\t{best_raw_dev:.8}");
        println!("v0370_best_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0370_best_consensus_dev_ccs_mae\t{best_consensus_dev:.8}");
        println!("v0370_best_consensus_ccs_ratio\t{consensus_ratio:.8}");
        println!(
            "v0370_rt_stretch_target_met\t{}",
            yes_no(
                best_protected_dev
                    .rt_mae_native
        ...
        println!(
            "v0370_ccs_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_TARGET_MAE_V0360)
        );
        println!(
            "v0370_ccs_stretch_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_STRETCH_TARGET_MAE_V0360)
        );
        if material {
            println!("v0370_material_dev_gain\tYES");
            println!("v0370_finalize_required\tYES");
            println!("v0370_rethink_required\tNO");
        } else {
            println!("v0370_material_dev_gain\tNO");
            println!("v0370_finalize_required\tNO");
            println!("v0370_rethink_required\tYES");
...
fn finite_ccs_indices(records: &[FoundationTrainingRecord], indices: &[usize]) -> Vec<usize> {
    indices
        .iter()
        .copied()
        .filter(|&index| records[index].ccs.is_some_and(|value| value.is_finite()))
        .collect()
}
...
fn collect_ccs_groups(
    records: &[FoundationTrainingRecord],
    provenance: &[FoundationRecordProvenance],
    indices: &[usize],
) -> Result<BTreeMap<String, Vec<CcsObservation>>> {
    let mut groups = BTreeMap::<String, Vec<CcsObservation>>::new();
    for &index in indices {
        let record = records
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.37 record index {index} outside corpus"))?;
        let Some(target) = record.ccs.filter(|value| value.is_finite()) else {
            continue;
        };
        let source = provenance
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.37 missing provenance for record {index}"))?;
        groups
...
fn build_train_consensus_supervision(
    ...
    provenance: &[FoundationRecordProvenance],
    indices: &[usize],
) -> Result<ConsensusSupervision> {
    let groups = collect_ccs_groups(records, provenance, indices)?;
    ...
    for source in all_sources {
        let examples = affine_examples
            ...
            .map(Vec::as_slice)
            .unwrap_or(&[]);
        let (affine, shared_identities) = if examples.len() >= V037_MIN_SOURCE_SHARED_IDENTITIES {
            let raw = fit_xy_affine(examples)?;
            let blend = examples.len() as f64 / (examples.len() as f64 + V037_SOURCE_SHRINKAGE);
            (
                AffineFit {
                    intercept: (blend * raw.intercept).clamp(-25.0, 25.0),
                    slope: (1.0 + blend * (raw.slope - 1.0)).clamp(0.85, 1.15),
                },
                examples.len(),
            )
        } else {
    ...
    for supervision in source_supervision.values_mut() {
        ...
        if count >= V037_MIN_SOURCE_SHARED_IDENTITIES && supervision.residual_mae.is_finite() {
    ...
    let global_residual_scale = if residual_maes.is_empty() {
        ...
        quantile_sorted(&residual_maes, 0.5).max(0.25)
    };
    for supervision in source_supervision.values_mut() {
        supervision.reliability =
            if supervision.shared_identities < V037_MIN_SOURCE_SHARED_IDENTITIES {
                0.60
            } else {
                (2.0 / (1.0 + supervision.residual_mae / global_residual_scale))
                    .clamp(V037_RELIABILITY_FLOOR, V037_RELIABILITY_CEILING)
            };
    }
...
fn build_consensus_from_groups(
    groups: &BTreeMap<String, Vec<CcsObservation>>,
    source_supervision: &BTreeMap<String, SourceSupervision>,
) -> Result<(Vec<ConsensusExample>, usize, usize, f64, f64)> {
    let mut examples = Vec::<ConsensusExample>::with_capacity(groups.len());
    ...
    for (identity, observations) in groups {
        ...
        for (family, mut views) in family_views {
            ...
            let representative = views
                ...
                .map(|entry| entry.2)
                .ok_or_else(|| anyhow::anyhow!("v0.37 family has no representative"))?;
        ...
        let clip_radius = (3.0 * mad).max(2.0);
        ...
        let weight = if source_count == 1 {
            ...
            (V037_SINGLETON_WEIGHT_SCALE * mean_reliability).clamp(0.25, 0.80)
        } else {
            multisource_examples += 1;
            let dispersion_factor = 1.0 / (1.0 + dispersion / V037_CONSENSUS_DISPERSION_SCALE);
        ...
        let representative_index = adjusted
            ...
            .map(|entry| entry.3)
            .ok_or_else(|| anyhow::anyhow!("v0.37 consensus identity has no representative"))?;
        total_weight += weight;
        examples.push(ConsensusExample {
            representative_index,
            target_ccs: consensus as f32,
...
fn consensus_fingerprint(examples: &[ConsensusExample]) -> u64 {
    let mut hash = 0xcbf29ce484222325u64;
    for example in examples {
        for value in [
            example.representative_index as u64,
            u64::from(example.target_ccs.to_bits()),
...
fn write_supervision_summary(output_root: &Path, supervision: &ConsensusSupervision) -> Result<()> {
    ...
    fs::write(
        output_root.join("ccs_source_reliability_v0370.tsv"),
        source_lines,
    )?;
    let summary = format!(
        "raw_train_ccs_records\t{}\nconsensus_train_examples\t{}\nmultisource_examples\t{}\nsingleton_examples\t{}\nmean_weight\t{:.8}\nmean_abs_adjusted_delta_to_consensus\t{:.8}\nsupervision_fingerprint\tfnv1a64:{:016x}\n",
    ...
    fs::write(
        output_root.join("ccs_consensus_supervision_v0370.tsv"),
        summary,
    )?;
    Ok(())
}

fn consensus_scalar_loss(
    ...
    let mut owned = Vec::<FoundationTrainingRecord>::with_capacity(examples.len());
    let mut weights = Vec::<f32>::with_capacity(examples.len());
    for example in examples {
        let mut record = records
            .get(example.representative_index)
            .ok_or_else(|| anyhow::anyhow!("v0.37 representative index outside corpus"))?
            .clone();
        record.ccs = Some(example.target_ccs);
        owned.push(record);
        weights.push(example.weight);
    }
    let mut batch = collator.collate(&owned, device, seed)?;
    ...
    let (target, mask) = match (batch.targets.ccs.as_ref(), batch.targets.ccs_mask.as_ref()) {
        (Some(target), Some(mask)) => (target, mask),
        _ => anyhow::bail!("v0.37 consensus training batch contains no CCS labels"),
    };
    let weight = Tensor::from_vec(weights, (examples.len(), 1), device)?;
    let mse = weighted_scaled_mse(
        &output.ccs,
        target,
        mask,
        &weight,
    ...
    let robust = weighted_scaled_pseudo_huber(
        &output.ccs,
        target,
        mask,
        &weight,
    ...
    Ok((mse.affine(V037_CCS_MSE_WEIGHT, 0.0)? + robust.affine(V037_CCS_ROBUST_WEIGHT, 0.0)?)?)
}

fn calibrate_consensus_residual_scale(
    model: &PeptideFoundationMultimodalV0360Model,
    collator: &FoundationCollator,
    records: &[FoundationTrainingRecord],
    examples: &[ConsensusExample],
    ...
    for chunk in examples.chunks(batch_size) {
        let mut owned = Vec::<FoundationTrainingRecord>::with_capacity(chunk.len());
        let mut weights = Vec::<f32>::with_capacity(chunk.len());
        for example in chunk {
            let mut record = records[example.representative_index].clone();
            record.ccs = Some(example.target_ccs);
            owned.push(record);
            weights.push(example.weight);
        }
        ...
        let output = model.scalar_v0360_t(&batch.input, &batch.context, &physics, false)?;
        let (Some(target), Some(mask)) =
            (batch.targets.ccs.as_ref(), batch.targets.ccs_mask.as_ref())
        else {
            continue;
        };
        let weight = Tensor::from_vec(weights, (chunk.len(), 1), device)?;
        let combined = mask.broadcast_mul(&weight)?;
        let squared = (&output.ccs - target)?
            .sqr()?
            .broadcast_mul(&combined)?
            .sum_all()?;
        let count = combined.sum_all()?;
        squared_error_sum += f64::from(squared.to_scalar::<f32>()?);
        weight_sum += f64::from(count.to_scalar::<f32>()?);
    }
    if !(weight_sum > 0.0 && squared_error_sum.is_finite()) {
        anyhow::bail!("v0.37 consensus TRAIN loss calibration has no finite weighted labels");
    }
    Ok((squared_error_sum / weight_sum)
        .sqrt()
        .max(V037_MIN_LOSS_SCALE))
}
...
fn weighted_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.37 weighted scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
    if !(scale > 0.0 && scale.is_finite()) {
        anyhow::bail!("v0.37 invalid CCS loss scale {scale}");
    }
    let combined = mask
...
fn weighted_scaled_pseudo_huber(
    ...
    if !(scale > 0.0 && scale.is_finite() && delta > 0.0 && delta.is_finite()) {
        anyhow::bail!("v0.37 invalid robust loss scale={scale} delta={delta}");
    }
    let combined = mask
        .broadcast_mul(weight)?
        .broadcast_as(prediction.dims())?;
    ...
    let examples = indices
        ...
        .filter_map(|&index| {
            records[index]
                ...
                .map(|target| ConsensusExample {
                    representative_index: index,
                    target_ccs: target,
                    weight: 1.0,
                    source_count: 1,
                    identity_hash: index as u64,
                })
        })
        .collect::<Vec<_>>();
    evaluate_ccs_consensus(
        model,
        collator,
        records,
        &examples,
...
fn evaluate_ccs_consensus(
    ...
    for chunk in examples.chunks(batch_size) {
        ...
        let target = batch
            ...
            .ccs
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("v0.37 evaluation batch lacks CCS targets"))?;
        let target = normalization
            .ccs
            .denormalize_tensor(target)?
            .to_vec2::<f32>()?;
        let mask = batch
            .targets
            .ccs_mask
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("v0.37 evaluation batch lacks CCS mask"))?
            .to_vec2::<f32>()?;
    ...
    if count == 0 {
        anyhow::bail!("v0.37 CCS evaluation contains no labels");
    }
    Ok(absolute_error / count as f64)
}

fn combined_dev_objective(raw_ratio: f64, consensus_ratio: f64) -> f64 {
    V037_RAW_OBJECTIVE_WEIGHT * raw_ratio + V037_CONSENSUS_OBJECTIVE_WEIGHT * consensus_ratio
}
...
fn masked_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.37 scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
...
fn validate_rt_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        if (current - initial).abs() > V037_RT_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.37 changed protected {label}: baseline={initial:.8} current={current:.8}"
            );
        }
    }
    Ok(())
...
fn validate_ms2_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        let current = current.ok_or_else(|| anyhow::anyhow!("missing {label} metric"))?;
        let initial = initial.ok_or_else(|| anyhow::anyhow!("missing baseline {label} metric"))?;
        if (current - initial).abs() > V037_MS2_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.37 changed protected {label}: baseline={initial:.8} current={current:.8}"
            );
        }
...
fn load_v0350_parent_variables(
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.37 VarMap lock poisoned during warm start"))?;
        ...
        match tensors.get(name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.37 warm-start shape mismatch for {name}: parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing.is_empty() {
        anyhow::bail!(
            "v0.35 parent is missing required v0.37 anchor variables: {}",
            missing.join(", ")
        );
    }
    ...
    drop(data);
    if loaded == 0 || fresh == 0 {
        anyhow::bail!("v0.37 warm start is nonfunctional: loaded={loaded} fresh={fresh}");
    }
    Ok(WarmStartReport {
...
fn read_v037_metadata(checkpoint: &Path) -> Result<V037Metadata> {
    let path = checkpoint.join("metadata.yaml");
    serde_yaml::from_str(
        &fs::read_to_string(&path).with_context(|| format!("failed to read {path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.37 metadata {path:?}"))
}

#[allow(clippy::too_many_arguments)]
fn validate_metadata(
    metadata: &V037Metadata,
    ...
    if metadata.version != V037_VERSION || metadata.objective != V037_OBJECTIVE {
        anyhow::bail!("v0.37 checkpoint identity mismatch");
    }
    if !(metadata.ccs_loss_scale_model_space > 0.0
        && metadata.ccs_loss_scale_model_space.is_finite())
    {
        anyhow::bail!("v0.37 checkpoint has invalid CCS loss scale");
    }
    ...
    {
        anyhow::bail!("v0.37 checkpoint data/supervision provenance mismatch");
    }
    if metadata.v0360_config != *config {
        anyhow::bail!("v0.37 checkpoint architecture mismatch");
    }
    if metadata.batch_size != batch_size
        || metadata.seed != seed
        || metadata.train_steps != train_steps
    {
        anyhow::bail!("v0.37 checkpoint run-contract mismatch");
    }
    if (metadata.learning_rate - learning_rate).abs()
        > f64::EPSILON * 64.0 * learning_rate.abs().max(1.0)
    {
        anyhow::bail!("v0.37 checkpoint learning-rate mismatch");
    }
    Ok(())
}
...
fn feasible_validation_batches(
    ...
    if max_batches == 0 {
        anyhow::bail!("v0.37 {label} cannot form one full batch");
    }
    if config.validation_source_weights.is_empty() {
    ...
    for &index in validation_indices {
        let source = provenance
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.37 {label} provenance index out of bounds"))?;
    ...
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.37 {label} validation weights are invalid");
    }
    ...
    anyhow::bail!("v0.37 {label} cannot satisfy source quotas for one full batch")
}
...
fn require_plan_records(label: &str, plan: &FoundationSamplePlan, expected: usize) -> Result<()> {
    if plan.indices.len() != expected {
        anyhow::bail!(
            "v0.37 {label} sample plan has {} records but {expected} are required",
            plan.indices.len()
        );
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v0370_affine_fit_recovers_linear_map() {
    ...
    #[test]
    fn v0370_consensus_prefers_reliable_source() {
        ...
        assert!(f64::from(examples[0].target_ccs) < 110.0);
        assert!(f64::from(examples[0].target_ccs) > 100.0);
    }

    #[test]
    fn v0370_epoch_order_is_deterministic_and_unique_within_cycle() {
        let examples = (0..32)
            .map(|index| ConsensusExample {
                representative_index: index,
                target_ccs: index as f32,
                weight: 1.0,
                source_count: 1,
