//! Frozen v0.24 inference-only TEST evaluator.
//!
...
//! The accepted v0.24 relation checkpoint and frozen v0.13.23 proposal pool are
//! scored without any optimization. TEST labels are used only after all candidate
//! scores/ranks are fixed, solely to report final held-out metrics.
...
const VERSION: &str = "v0.24.0-test-eval";
...
fn main() -> Result<()> {
    ...
    if args.len() != 5 {
        anyhow::bail!(
            "usage: foundation_score_fragment_relation_v0240 RUN.yaml UNIFIED_CHECKPOINT RELATION_CHECKPOINT_DIR TEST_CANDIDATES.tsv OUTPUT_DIR"
        );
    }
    let run_yaml = PathBuf::from(&args[0]);
    let unified_checkpoint = PathBuf::from(&args[1]);
    let relation_checkpoint = PathBuf::from(&args[2]);
    let test_tsv = PathBuf::from(&args[3]);
    ...
    if acknowledgement != "OPEN_FROZEN_TEST_ONCE_V0240_31_47" {
        anyhow::bail!(
            "TEST scoring is locked. Set REDEEM_V0240_TEST_ACK=OPEN_FROZEN_TEST_ONCE_V0240_31_47 only for the accepted one-shot evaluation"
        );
    }
    if output_dir.exists() {
        anyhow::bail!("v0.24 TEST output must be fresh: {}", output_dir.display());
    }

    let started = Instant::now();
    stage(started, "start");
    println!("version\t{VERSION}");
    println!("mode\tfrozen_test_inference_only");
    ...
    println!("global_energy\tfixed_negative_log_legacy_rank_prior_plus_frozen_fragment_relation_residual");
    println!("test_partition_consumed\ttrue");

    let relation_metadata_path = relation_checkpoint.join("metadata.yaml");
    let relation_metadata: RelationMetadata = serde_yaml::from_str(
        &fs::read_to_string(&relation_metadata_path)
            .with_context(|| format!("read frozen relation metadata {relation_metadata_path:?}"))?,
    )?;
    if relation_metadata.version != "v0.24.0"
        || relation_metadata.run_mode != "full"
        || relation_metadata.architecture != FOUNDATION_FRAGMENT_RELATION_ARCHITECTURE_V0240
        || relation_metadata.objective != FOUNDATION_FRAGMENT_RELATION_OBJECTIVE_V0240
        || relation_metadata.candidate_labels_used_as_model_features
        || relation_metadata.legacy_rank_used_as_model_feature
        || relation_metadata.test_partition_consumed
        || relation_metadata.relation_feature_dim != FOUNDATION_FRAGMENT_RELATION_FEATURE_DIM_V0240
        || relation_metadata.train_steps != FULL_TRAIN_STEPS
        || relation_metadata.anchor_batch != FULL_ANCHOR_BATCH
        || relation_metadata.list_width != PROPOSAL_LIST_WIDTH
        || relation_metadata.final_validation_records != REQUIRED_RECORDS
        || relation_metadata.final_literal_top1 != 31
        || relation_metadata.final_il_top1 != 47
        || relation_metadata.validation_oracle_literal != REQUIRED_ORACLE_LITERAL
        || relation_metadata.validation_oracle_il != REQUIRED_ORACLE_IL
        || relation_metadata.stop_rule
            != "V0240_PROGRESS_TARGET_MET_HARD_NEGATIVE_FRAGMENT_RELATION"
    {
        anyhow::bail!(
            "relation checkpoint is not the accepted frozen v0.24 31/47 validation milestone"
        );
    }
    let relation_model_path = relation_checkpoint.join("fragment_relation_model.safetensors");
    if !relation_model_path.is_file() {
        anyhow::bail!(
            "missing frozen relation model: {}",
            relation_model_path.display()
        );
    }
    stage(started, "frozen_relation_contract_verified");
    ...
    let test_groups = read_candidate_groups(&test_tsv, true)
        .with_context(|| format!("read frozen TEST candidate TSV {test_tsv:?}"))?;
    verify_partition(&test_groups, &benchmark, FoundationPartition::Test, "TEST")?;
    if test_groups.is_empty() {
        anyhow::bail!("frozen TEST candidate TSV contains no candidate groups");
    }
    stage(started, "test_candidates_verified");
    ...
    if predictor.metadata.forward_config.model_dim != 96
        || predictor.metadata.forward_config.max_sequence_len != ACCEPTED_MAX_SEQUENCE_LEN
        || predictor.metadata.forward_config.ms2_fragment_channels < 4
    {
        anyhow::bail!("frozen unified forward checkpoint is incompatible with accepted v0.24");
    }
    ...
    let mut relation_varmap = VarMap::new();
    ...
    relation_varmap
        .load(&relation_model_path)
        .with_context(|| format!("load frozen relation model {relation_model_path:?}"))?;
    println!("relation_checkpoint\t{}", relation_checkpoint.display());
    println!(
        "relation_model_fingerprint\t{}",
        relation_model_fingerprint(&relation_varmap)?
    );
    stage(started, "frozen_relation_model_loaded");

    fs::create_dir_all(&output_dir)?;
    let diagnostics_path = output_dir.join("test_relation.tsv");
    ...
    stage(started, "test_scoring_complete");

    print_validation_metrics(&metrics);
    println!("accepted_validation_literal_top1\t31");
    println!("accepted_validation_il_top1\t47");
    println!("test_relation_diagnostics\t{}", diagnostics_path.display());
    println!("v0240_test_stop_rule\tFROZEN_TEST_REPORTED_DO_NOT_TUNE_ON_TEST");
    println!("test_partition_consumed\ttrue");
...
fn read_candidate_groups(
    ...
    if require_proposal_export {
        for required in [
            ...
            if !index.contains_key(required) {
                anyhow::bail!(
                    "v0.24 frozen proposal TSV missing frozen-generator provenance column '{required}'"
                );
            }
        }
    }
