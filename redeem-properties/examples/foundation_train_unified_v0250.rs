//! v0.25 GPU re-foundation: random-initialized unified model trained to TRAIN-dev convergence.
//!
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 10 {
        anyhow::bail!(
            "usage: foundation_train_unified_v0250 RUN_V0250.yaml OUTPUT_DIR ARCHITECTURE_TEMPLATE_UNIFIED [max_epochs=30] [batch_size=32] [patience=5] [min_delta=0.002] [seed=20260925] [learning_rate=1e-4]"
        );
    }
    ...
    if output_root.exists() {
        anyhow::bail!("v0.25 output directory already exists: {:?}", output_root);
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.25 requires a CUDA device")?;
    ...
    // Freeze the accepted v0.13.10 MS2 formulation while retraining all
    ...
    let forward_objective_weight = 0.50f64;
    let diffusion_objective_weight = 0.125f64;
    let causal_objective_weight = 0.375f64;
    ...
    println!("v0250_version\tv0.25.0-gpu-random-init-convergence");
    println!("objective\tunified_forward_diffusion_causal_alignment_v01310_frozen_objective");
    ...
    let metadata = |completed_steps| UnifiedPilotMetadata {
        version: 25,
        objective: "v0250_random_init_unified_v01310_objective_train_derived_dev".into(),
    ...
    for epoch in 1..=max_epochs {
        ...
        println!("v0250_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
        ...
        for local_step in 0..steps_per_epoch {
            ...
            if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                println!(
                    "v0250_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={total_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                    local_step + 1,
        ...
        if improved {
            ...
            println!("v0250_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}");
        } else {
            stale_epochs += 1;
        }

        println!("v0250_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}");
        if stale_epochs >= patience {
            stopped_early = true;
            println!("v0250_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}");
            break;
        }
    }
    ...
    varmap
        .load(&best_model)
        .with_context(|| format!("failed to restore best v0.25 model {best_model:?}"))?;
    ...
    println!("historical_validation_consumed\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0250_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");
...
fn normalized_dev_objective(
    ...
    push_ratio(&mut ratios, p.rt_mae_native, bp.rt_mae_native);
    push_ratio(&mut ratios, p.ccs_mae_native, bp.ccs_mae_native);
    push_ratio(&mut ratios, p.ms2_loss, bp.ms2_loss);
    push_ratio_value(&mut ratios, i.diffusion_loss, bi.diffusion_loss);
    push_ratio_value(&mut ratios, i.causal_loss, bi.causal_loss);
    push_ratio_value(&mut ratios, i.alignment_loss, bi.alignment_loss);
    if ratios.len() < 4 {
        anyhow::bail!(
            "v0.25 dev objective has too few finite components: {}",
            ratios.len()
        );
    }
...
fn copy_checkpoint_dir(source: &Path, destination: &Path) -> Result<()> {
    ...
    for name in [
        ...
        fs::copy(source.join(name), destination.join(name)).with_context(|| {
            format!("failed to copy v0.25 checkpoint file {name} from {source:?}")
        })?;
    }
    Ok(())
}
