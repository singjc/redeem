//! Validation-only three-state audit for the v0.13.8 MS2 activation rescue.
//!
//! This compares, on exactly the same source-balanced VALIDATION records:
//! 1. the accepted v0.13.7 ReLU checkpoint;
//! 2. the v0.13.8 zero-step checkpoint with identical weights and Softplus-v0.13.8;
//! 3. the trained v0.13.8 final checkpoint.
//!
...
fn main() -> Result<()> {
    ...
    if !(5..=8).contains(&args.len()) {
        bail!(
            "usage: foundation_benchmark_ms2_activation <training.yaml> <reference_relu_checkpoint> <candidate_initial_checkpoint> <candidate_final_checkpoint> <output_dir> [max_records_per_source=512] [batch_size=32] [seed=20260912]"
        );
    }
    ...
    if selected.is_empty() {
        bail!("no VALIDATION records selected for MS2 activation audit");
    }
    ...
    {
        bail!("reference checkpoint must use historical relu MS2 activation");
    }
    if initial_metadata.forward_config.ms2_output_activation
        != FoundationMs2OutputActivation::SoftplusV0138
        || final_metadata.forward_config.ms2_output_activation
            != FoundationMs2OutputActivation::SoftplusV0138
    {
        bail!("candidate initial/final checkpoints must use softplus-v0138 MS2 activation");
    }
    ...
    let records_path = output_dir.join("ms2_activation_records.tsv");
    let fragments_path = output_dir.join("ms2_activation_fragments.tsv");
    let source_summary_path = output_dir.join("ms2_activation_source_summary.tsv");
    let family_summary_path = output_dir.join("ms2_activation_family_summary.tsv");
    let channel_summary_path = output_dir.join("ms2_activation_channel_summary.tsv");
    let paired_summary_path = output_dir.join("ms2_activation_paired_summary.tsv");
    ...
    let states = [
        ("reference_relu_v0137", &reference_checkpoint),
        ("initial_softplus_v0138", &initial_checkpoint),
        ("final_softplus_v0138", &final_checkpoint),
    ];
    ...
    let comparisons = [
        (
            "activation_effect",
            "reference_relu_v0137",
            "initial_softplus_v0138",
        ),
        (
            "training_effect",
            "initial_softplus_v0138",
            "final_softplus_v0138",
        ),
        ("net_effect", "reference_relu_v0137", "final_softplus_v0138"),
    ];
    ...
    for label in [
        ...
        println!(
            "ms2_activation_summary\tcheckpoint={label}\tactivation={}\tcompleted_steps={}\trecords={}\tfragments={}\tpointwise_mse={}\tpointwise_mae={}\tcosine={}\tspectral_angle={}\tpearson={}\texact_zero_fraction={}\tnear_zero_1e4_fraction={}\tnear_zero_1e3_fraction={}\tnear_zero_1e2_fraction={}\tmean_target_intensity={}\tmean_predicted_intensity={}",
    ...
    for comparison in ["activation_effect", "training_effect", "net_effect"] {
