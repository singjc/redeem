...
use redeem_properties::foundation::{
    ...
    FoundationFragmentContextBatchV0270, FoundationPartition, FoundationTargetNormalizationConfig,
    FoundationTrainingRecord, PeptideFoundationMultimodalV0270Config,
    PeptideFoundationMultimodalV0270Model, RetentionTimeObjective,
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0270,
};
...
const VERSION: &str = "v0.27-alphapeptdeep-comparison-export";
...
fn main() -> Result<()> {
    ...
    if !(3..=6).contains(&args.len()) {
        anyhow::bail!(
            "usage: foundation_export_alphapeptdeep_comparison_v0270 RUN.yaml V0270_CHECKPOINT OUTPUT_DIR [validation_per_task=all_if_omitted] [rt_train_calibration=256] [seed=20260916]"
        );
    }
    ...
    if metadata.version != 270 {
        anyhow::bail!(
            "expected v0.27 checkpoint metadata version 270, observed {}",
            metadata.version
        );
    }
    ...
    let model = PeptideFoundationMultimodalV0270Model::new(metadata.v0270_config.clone(), vb)?;
    varmap
        .load(checkpoint.join("model.safetensors"))
        .with_context(|| format!("load v0.27 multimodal checkpoint {checkpoint:?}"))?;
...
fn predict_records(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let mut prediction = model
            .forward()
            .forward_v0270_t(&batch.input, &batch.context, &fragment_context, false)?
            .base;
