...
use redeem_properties::foundation::{
    ...
    FoundationFragmentContextBatchV0310, FoundationPartition, FoundationTargetNormalizationConfig,
    FoundationTrainingRecord, PeptideFoundationMultimodalV0310Config,
    PeptideFoundationMultimodalV0310Model, RetentionTimeObjective,
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0310,
};
...
const VERSION: &str = "v0.31-alphapeptdeep-comparison-export";
...
fn main() -> Result<()> {
    ...
    if !(3..=6).contains(&args.len()) {
        anyhow::bail!(
            "usage: foundation_export_alphapeptdeep_comparison_v0310 RUN.yaml V0310_CHECKPOINT OUTPUT_DIR [validation_per_task=all_if_omitted] [rt_train_calibration=256] [seed=20260916]"
        );
    }
    ...
    if metadata.version != 310 {
        anyhow::bail!(
            "expected v0.31 checkpoint metadata version 310, observed {}",
            metadata.version
        );
    }
    ...
    let model = PeptideFoundationMultimodalV0310Model::new(metadata.v0310_config.clone(), vb)?;
    varmap
        .load(checkpoint.join("model.safetensors"))
        .with_context(|| format!("load v0.31 multimodal checkpoint {checkpoint:?}"))?;
...
fn predict_records(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let mut prediction = model
            .forward_v0310_t(&batch.input, &batch.context, &fragment_context, false)?
            .base;
