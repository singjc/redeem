//! Train the v0.17.0 end-to-end spectrum-peptide compatibility representation.
//!
//! This experiment is the representation/pretraining-level response to the
//! rejected v0.15/v0.16 post-hoc interaction heads. It does not regenerate
...
const HARD_NEGATIVES_PER_ANCHOR: usize = 7;
const HARD_NEGATIVE_POOL: usize = 24;
...
impl RunMode {
    fn parse(value: Option<&str>) -> Result<Self> {
        match value.unwrap_or("full").trim().to_ascii_lowercase().as_str() {
            "full" => Ok(Self::Full),
            "smoke" => Ok(Self::Smoke),
            other => anyhow::bail!("unsupported v0.17.0 mode {other:?}; expected full or smoke"),
        }
    }
...
fn main() -> Result<()> {
    ...
    if validation_groups.is_empty() {
        anyhow::bail!("v0.17.0 validation candidate TSV contains no mass-valid groups");
    }
    verify_partition(
        &validation_groups,
        &benchmark,
    ...
    if metadata.forward_config.model_dim != 96 || metadata.inverse_config.model_dim != 96 {
        anyhow::bail!(
            "v0.17.0 is anchored to the accepted 96-d unified checkpoint; forward={} inverse={}",
    ...
    if metadata.forward_config.model_dim != metadata.inverse_config.model_dim {
        anyhow::bail!("v0.17.0 requires matching forward/inverse model widths");
    }
    ...
    if hard_groups.len() < mode.anchor_batch() {
        anyhow::bail!(
            "v0.17.0 hard-negative mining produced only {} groups, fewer than anchor batch {}",
            hard_groups.len(),
            mode.anchor_batch()
        );
    }
    if mode == RunMode::Full && hard_groups.len() < MIN_FULL_HARD_NEGATIVE_GROUPS {
        anyhow::bail!(
            "v0.17.0 full pretraining requires at least {MIN_FULL_HARD_NEGATIVE_GROUPS} TRAIN hard-negative groups; observed {}. Inspect TRAIN-only mining statistics and revise the representation-pretraining data construction before any scientific validation run",
            hard_groups.len()
        );
    }
    ...
    println!("objective\tpositive_first_listwise_precursor_compatible_hard_negative_pretraining");
    ...
    if mode == RunMode::Full {
        println!(
            "v0170_representation_gate\trequired_literal={}\trequired_il={}\trequired_oracle_literal={}\trequired_oracle_il={}\tobserved_literal={}\tobserved_il={}\tobserved_oracle_literal={}\tobserved_oracle_il={}\tgate={}",
        ...
        println!(
            "v0170_stop_rule\t{}",
            if gate {
                "ACCEPT_END_TO_END_COMPATIBILITY_REPRESENTATION_AND_FREEZE_V0170"
            } else {
                "V0170_REPRESENTATION_GATE_NOT_MET_DIAGNOSE_PRETRAINING_SIGNAL"
            }
        );
    } else {
        println!("v0170_representation_gate\tNOT_EVALUATED_SMOKE_RUNTIME_ONLY");
        println!("v0170_stop_rule\tSMOKE_RUNTIME_ONLY_NO_SCIENTIFIC_DECISION");
    }
    ...
    let metadata_out = CompatibilityMetadata {
        ...
        objective: "positive_first_listwise_precursor_compatible_hard_negative_pretraining".into(),
...
fn build_hard_negative_groups(
    ...
    if groups.is_empty() {
        anyhow::bail!(
            "v0.17.0 found no TRAIN spectrum anchors with at least {HARD_NEGATIVES_PER_ANCHOR} non-I/L hard negatives within +/-{HARD_NEGATIVE_MASS_WINDOW_DA:.3} Da"
        );
    }
...
fn build_training_batch(
    ...
    if peptides.len() != group_indices.len() * group_width {
        anyhow::bail!("v0.17.0 internal hard-negative group width mismatch");
    }
...
fn validate_candidate_contract(groups: &[CandidateGroup]) -> Result<ValidationContract> {
    ...
    {
        anyhow::bail!(
            "v0.17.0 frozen candidate oracle/window mismatch: oracle={}/{} window={}/{} expected={}/{}",
    ...
    {
        anyhow::bail!(
            "v0.17.0 frozen legacy baseline mismatch: observed={}/{} expected={}/{}",
...
fn verify_partition(
    ...
    for group in groups {
        if !allowed.contains(&group.record_index) {
            anyhow::bail!(
                "v0.17.0 {label} candidate group {} is not assigned to expected benchmark partition",
                group.record_index
            );
        }
    }
...
fn reject_test_path(path: &Path) -> Result<()> {
    ...
    if suspicious {
        anyhow::bail!("v0.17.0 forbids TEST-partition inputs; suspicious path {path:?}");
    }
    Ok(())
}
...
fn exported_peptidoform(row: &CandidateRow) -> Result<PeptidoformInput> {
        ...
        .filter(|part| !part.trim().is_empty())
    {
            ...
            .and_then(|value| value.strip_suffix(')'))
        {
            ...
            anyhow::bail!(
                "v0.17.0 cannot reconstruct unsupported exported modification site '{site}'"
            );
        }
    }
    nterm.sort_unstable();
...
fn require_nonzero_end_to_end_gradients(
    ...
    for (label, prefix) in [
        ...
        if !(norm > 0.0 && norm.is_finite()) {
            anyhow::bail!(
                "v0.17.0 end-to-end gradient probe failed for {label}: gradient_norm={norm}"
            );
        }
    }
    Ok(())
