//! v0.35 trainable forward-representation continuation from frozen v0.31.
//!
//! v0.34 falsified the hypothesis that larger heads on a frozen v0.31
//! representation can close the remaining RT/MS2 gap. v0.35 instead clones the
//! accepted v0.31 forward encoder/refinement/heads into a separate trainable
//! branch, adapts that representation end-to-end, injects acquisition context
//! before an MS2 residue Transformer, and restores masked/chemistry/contrastive
//! representation objectives as auxiliary supervision. The original v0.31 model
//! remains immutable for CCS and inverse diagnostics.
//!
//! Historical VALIDATION has already been consumed descriptively by v0.31 and is
//! not reused for v0.35 selection. Historical TEST remains closed.
...
use redeem_properties::foundation::{
    ...
    foundation_fragment_representation_aux_loss_v0350, foundation_ms2_loss,
    foundation_multimodal_ms2_loss_v0350, foundation_multimodal_relation_margin_loss_v0350,
    ...
    FoundationDiffusionConfig, FoundationDiffusionVocabulary, FoundationFragmentContextBatchV0350,
    FoundationLearningRateSchedule, FoundationLossWeights, FoundationMs2LossConfig,
    FoundationMs2OutputActivation, FoundationMultiTaskOutput, FoundationPartition,
    ...
    FoundationTrainingViews, PeptideFoundationMultimodalV0310Config,
    PeptideFoundationMultimodalV0350Config, PeptideFoundationMultimodalV0350Model,
    PeptidoformInput, PrecursorContextBatch, RetentionTimeObjective,
    FOUNDATION_DIFFUSION_VOCAB_SIZE, FOUNDATION_FRAGMENT_AUX_WEIGHT_V0350,
    FOUNDATION_FRAGMENT_RELATION_CORE_CHANNELS_V0240,
    FOUNDATION_FRAGMENT_RELATION_FEATURE_DIM_V0240,
    FOUNDATION_FRAGMENT_RELATION_MATCHED_OFFSET_V0240, FOUNDATION_MS2_PEARSON_WEIGHT_V0350,
    FOUNDATION_MS2_SOFTPLUS_BETA_V0138, FOUNDATION_MULTIMODAL_ARCHITECTURE_V0350,
    FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0350, FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0350,
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0350 RUN_V0260.yaml OUTPUT_DIR PARENT_V0310_CHECKPOINT [max_epochs=8] [batch_size=64] [patience=3] [min_delta=0.003] [seed=20261035] [learning_rate=3e-5] [mode=train|resume|finalize]"
        );
    }
    ...
    let min_delta = parse_or(&args, 7, 0.003f64)?;
    let seed = parse_or(&args, 8, 20_261_035u64)?;
    ...
    let (resume_training, finalize_only) = match run_mode {
        ...
        "finalize" => (false, true),
        other => anyhow::bail!(
            "unsupported v0.35 run mode {other:?}; expected train, resume, or finalize"
        ),
    };
    ...
    if finalize_only || resume_training {
        if !output_root.is_dir() {
            anyhow::bail!(
                "v0.35 {run_mode} mode requires an existing model output directory: {:?}",
                output_root
            );
        }
        ...
        for checkpoint in required_checkpoints {
            for name in [
                ...
                if !path.is_file() {
                    anyhow::bail!("v0.35 {run_mode} mode is missing checkpoint file: {path:?}");
                }
            }
        }
    } else if output_root.exists() {
        anyhow::bail!("v0.35 output directory already exists: {:?}", output_root);
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.35 requires a CUDA device")?;
    ...
    if parent_metadata.version != 310 {
        anyhow::bail!(
            "v0.35 requires the frozen v0.31 parent metadata version 310, observed {}",
    ...
    {
        anyhow::bail!(
            "v0.35 requires the accepted v0.31 objective, observed {}",
            parent_metadata.objective
        );
    }
    if parent_metadata.completed_steps == 0 {
        anyhow::bail!("v0.35 refuses an unselected v0.31 baseline checkpoint");
    }
    ...
    {
        anyhow::bail!("v0.35 parent v0.31 architecture config does not match its base configs");
    }
    let v0350_config =
        PeptideFoundationMultimodalV0350Config::fixed(parent_metadata.v0310_config.clone())?;

    // The v0.35 prepare step excludes peptides that cannot be represented by
    ...
    if let Some((record_index, sequence, sequence_len)) = first_overlength {
        anyhow::bail!(
            "v0.35 prepared benchmark contains {prepared_overlength_records} peptide(s) longer than architecture max_sequence_len={}; first record_index={record_index} length={sequence_len} sequence={sequence}",
    ...
    let model = PeptideFoundationMultimodalV0350Model::new(v0350_config.clone(), vb)?;

    // Restore the complete accepted v0.31 model exactly, then clone its
    // peptide-side encoder/refinement/RT/MS2/auxiliary weights into a separate
    // trainable forward namespace. Only the acquisition-context conditioner is
    // genuinely fresh, and its residual output is zero initialized. Step 0 is
    // therefore an exact functional copy of v0.31 for RT/MS2/CCS.
    ...
    let warm_start_description = format!(
        "v0310_frozen_anchor_plus_trainable_forward_clone_plus_identity_ms2_context_from={}",
    ...
    // v0.35 is a forward representation-learning experiment. The complete v0.31
    // anchor, including CCS/inverse/alignment/relation paths, remains frozen.
    ...
    println!("v0350_version\tv0.35-trainable-forward-representation");
    println!("objective\tv0350_trainable_forward_representation_context_conditioned_ms2");
    println!("architecture\t{}", FOUNDATION_MULTIMODAL_ARCHITECTURE_V0350);
    ...
    println!("random_initialization\tms2_context_v0350_only_identity_output");
    println!(
    ...
    println!("historical_validation_reused_for_v0350_selection\tNO");
    ...
    println!("optimizer_scope\tforward_v0350+property_refinement_v0350+ms2_context_v0350+fragment_aux_v0350_only");
    ...
    println!(
        "relation_margin\t{}",
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0350
    );
    println!(
        "property_adapter_hidden\t{}",
        FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0350
    );
    println!("forward_representation\ttrainable_clone_of_v0310_encoder_plus_property_refinement");
    println!("ms2_contextualization\tcharge_nce_instrument_before_3x192_transformer_then_v0310_fragment_decoder");
    println!(
        "fragment_deep_supervision_weight\t{}",
        FOUNDATION_FRAGMENT_AUX_WEIGHT_V0350
    );
    println!(
        "rt_robust_aux_weight\t{}",
        FOUNDATION_RT_ROBUST_WEIGHT_V0350
    );
    println!(
        "rt_robust_aux_delta_standardized\t{}",
        FOUNDATION_RT_ROBUST_DELTA_V0350
    );
    println!(
        "ms2_pearson_aux_weight\t{}",
        FOUNDATION_MS2_PEARSON_WEIGHT_V0350
    );
    println!(
        "representation_masked_weight\t{}",
        FOUNDATION_REPRESENTATION_MASKED_WEIGHT_V0350
    );
    println!(
        "representation_chemistry_weight\t{}",
        FOUNDATION_REPRESENTATION_CHEMISTRY_WEIGHT_V0350
    );
    println!(
        "representation_contrastive_weight\t{}",
        FOUNDATION_REPRESENTATION_CONTRASTIVE_WEIGHT_V0350
    );
    println!(
        "forward_training_input\tclean_supervised_plus_separate_corrupted_representation_views"
    );
    println!("accepted_parent\tcomplete_v0310_frozen");
    println!("base_encoder_update_policy\tfrozen_v0310_anchor_plus_trainable_forward_clone");
    println!("ccs_update_policy\texact_frozen_v0310_v0270_path");
    println!("training_objectives\tclean_rt_plus_factorized_ms2_plus_fragment_deep_supervision_plus_masked_chemistry_contrastive_representation");
    println!("dev_selection\trt_and_all_three_ms2_quality_metrics_must_improve");
    println!("dev_materiality_gate\tall_rt_ms2_quality_metrics_improve_and_rt_weighted_combined_error_objective_le_0.85");
    println!("rt_stretch_target_native_mae\t4.0");
    println!(
        "historical_validation_status\talready_consumed_by_v0310_not_available_for_v0350_selection"
    );
    ...
    let metadata = |completed_steps| UnifiedPilotMetadata {
        version: 350,
        objective: "v0350_trainable_forward_representation_context_conditioned_ms2".into(),
        ...
        relation_margin: FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0350,
        property_adapter_hidden: FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0350,
        ...
        v0350_config: v0350_config.clone(),
    ...
    if finalize_only {
        ...
        if best_metadata.version != 350 {
            anyhow::bail!(
                "v0.35 finalize mode expected checkpoint metadata version 350, observed {}",
                best_metadata.version
            );
        }
        if best_metadata.completed_steps == 0 {
            anyhow::bail!(
                "v0.35 has no DEV improvement over frozen v0.31; refusing to consume HOLDOUT/finalize baseline"
            );
        }
        if best_metadata.corpus_fingerprint != expected_corpus_fingerprint {
            anyhow::bail!(
                "v0.35 finalize corpus fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
            anyhow::bail!(
                "v0.35 finalize benchmark fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.batch_size != batch_size {
            anyhow::bail!(
                "v0.35 finalize batch_size mismatch: checkpoint={} requested={batch_size}",
                best_metadata.batch_size
            );
        }
        if best_metadata.seed != seed {
            anyhow::bail!(
                "v0.35 finalize seed mismatch: checkpoint={} requested={seed}",
                best_metadata.seed
            );
        }
        ...
        if !initial_model.is_file() {
            anyhow::bail!("v0.35 finalize requires the saved step-0 initial checkpoint");
        }
        varmap
            .load(&initial_model)
            .with_context(|| format!("failed to restore v0.35 initial model {initial_model:?}"))?;
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore best v0.35 model {best_model:?}"))?;
        ...
        println!("v0350_finalize_dev_objective\t{best_dev_objective:.8}");
        if best_dev_objective > 0.85 {
            anyhow::bail!(
                "v0.35 DEV gain is below the fixed materiality gate (objective={best_dev_objective:.8} > 0.85); refusing to consume HOLDOUT"
            );
        }
        ...
        println!("v0350_finalize_only\ttrue");
        println!("v0350_finalize_source_checkpoint\t{}", best_dir.display());
        println!("v0350_finalize_best_step\t{best_step}");
        println!(
            "v0350_finalize_best_epoch\t{}",
            best_epoch
        ...
        println!("v0350_finalize_complete\tbest_step={best_step}");
    ...
    let (
        ...
        for (label, checkpoint_metadata) in [
            ...
            validate_v0350_training_checkpoint_metadata(
                label,
                checkpoint_metadata,
                &expected_corpus_fingerprint,
                &expected_benchmark_fingerprint,
        ...
        if initial_metadata.completed_steps != 0 {
            anyhow::bail!(
                "v0.35 resume expected initial checkpoint at step 0, observed {}",
        ...
        {
            anyhow::bail!(
                "v0.35 resume requires latest checkpoint at a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                latest_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps % steps_per_epoch != 0 {
            anyhow::bail!(
                "v0.35 resume requires best checkpoint at baseline or a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                best_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps > latest_metadata.completed_steps {
            anyhow::bail!(
                "v0.35 resume best checkpoint step {} is newer than latest step {}",
                best_metadata.completed_steps,
                latest_metadata.completed_steps
            );
        }
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore v0.35 best model {best_model:?}"))?;
        ...
        varmap
            .load(&latest_model)
            .with_context(|| format!("failed to restore v0.35 latest model {latest_model:?}"))?;
        let latest_optimizer = latest_dir.join("optimizer.safetensors");
        optimizer
            .load_safetensors(&latest_optimizer)
            .with_context(|| {
                format!("failed to restore v0.35 optimizer state {latest_optimizer:?}")
            })?;
        ...
        if stale_epochs >= patience {
            println!(
                "v0350_resume_already_early_stopped\tcompleted_epoch={completed_epoch}\tbest_epoch={best_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
            );
        }
        println!(
            "v0350_resume\tlatest_epoch={completed_epoch}\tlatest_step={global_step}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstale_epochs={stale_epochs}"
        );
        (
            initial_metrics,
            global_step,
        ...
        // v0.35 is an identity-preserving continuation at the accepted output
    ...
    if start_epoch > max_epochs {
        println!(
            "v0350_training_budget_complete\tstart_epoch={start_epoch}\tmax_epochs={max_epochs}\tlatest_step={global_step}"
        );
    } else if stopped_early {
        println!(
            "v0350_training_already_early_stopped\tstart_epoch={start_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
        );
    } else {
        for epoch in start_epoch..=max_epochs {
            ...
            println!("v0350_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
            ...
            for local_step in 0..steps_per_epoch {
                ...
                if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                    println!(
                        "v0350_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={total_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                        local_step + 1,
            ...
            if improved {
                ...
                println!(
                    "v0350_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}"
                );
            } else {
                stale_epochs += 1;
            }
            ...
            println!(
                "v0350_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}"
            );
            if stale_epochs >= patience {
                stopped_early = true;
                println!(
                    "v0350_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}"
                );
                break;
            }
        }
    ...
    println!("historical_validation_reused_for_v0350_selection\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0350_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");
    if best_step == 0 {
        println!("v0350_no_dev_improvement_over_parent\tYES");
        println!("v0350_material_dev_gain\tNO");
        println!("v0350_finalize_required\tNO");
        println!("v0350_rethink_required\tYES");
    } else if best_objective <= 0.85 {
        println!("v0350_no_dev_improvement_over_parent\tNO");
        println!("v0350_material_dev_gain\tYES");
        println!("v0350_finalize_required\tYES");
        println!("v0350_rethink_required\tNO");
    } else {
        println!("v0350_no_dev_improvement_over_parent\tNO");
        println!("v0350_material_dev_gain\tNO");
        println!("v0350_finalize_required\tNO");
...
fn normalized_dev_objective(
    ...
    if (ccs - 1.0).abs() > 2.0e-4 {
        anyhow::bail!("v0.35 protected CCS drifted from the frozen step-0 path: ratio={ccs:.8}");
    }
    ...
    // A v0.35 representation checkpoint is eligible only when RT and every externally
...
fn quality_error_ratio(value: Option<f64>, baseline: Option<f64>, label: &str) -> Result<f64> {
    let (value, baseline) = match (value, baseline) {
        ...
        _ => anyhow::bail!("v0.35 dev objective is missing {label} metric"),
    };
    if !value.is_finite() || !baseline.is_finite() || baseline >= 1.0 {
        anyhow::bail!(
            "v0.35 dev objective has invalid {label}: value={value}, baseline={baseline}"
        );
    }
...
fn finite_ratio_value(value: f64, baseline: f64, label: &str) -> Result<f64> {
    if !value.is_finite() || !baseline.is_finite() || baseline.abs() <= 1.0e-12 {
        anyhow::bail!(
            "v0.35 dev objective has invalid {label} ratio: value={value}, baseline={baseline}"
        );
    }
    Ok(value / baseline)
}
...
fn copy_checkpoint_dir(source: &Path, destination: &Path) -> Result<()> {
    ...
    for name in [
        ...
        fs::copy(source.join(name), destination.join(name)).with_context(|| {
            format!("failed to copy v0.35 checkpoint file {name} from {source:?}")
        })?;
    }
    Ok(())
}

fn paired_masked_pseudo_huber_v0350(
    prediction: &Tensor,
    ...
    if !(delta > 0.0 && delta.is_finite()) {
        anyhow::bail!("v0.35 pseudo-Huber delta must be positive and finite");
    }
    match (target, mask) {
        (Some(target), Some(mask)) => {
            if prediction.dims() != target.dims() {
                anyhow::bail!(
                    "v0.35 RT robust loss shape mismatch: prediction {:?}, target {:?}",
                    prediction.dims(),
                    target.dims()
                );
            }
        ...
        (None, None) => Ok(None),
        _ => anyhow::bail!("v0.35 RT target/mask must be supplied together"),
    }
}

fn masked_pearson_loss_v0350(
    prediction: &Tensor,
    target: &Tensor,
    mask: &Tensor,
    epsilon: f64,
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.35 MS2 Pearson loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
...
fn forward_loss(
    ...
    representation_collator: &FoundationCollator,
    ...
    // Supervised RT/MS2 always see clean peptide chemistry. Representation
    // corruption is isolated to the auxiliary branch below rather than being
    // mixed into property labels.
    let mut clean_views = clean_collator.collate_views(records, device, seed)?;
    normalize_targets(&mut clean_views, normalization)?;
    let fragment_context =
        FoundationFragmentContextBatchV0350::from_records(records, model.forward_config(), device)?;
    let fragment_channel_mask = fragment_context.channel_mask()?;
    let first = model.forward_v0350_t(
        &clean_views.first.input,
        &clean_views.first.context,
        &fragment_context,
        true,
    )?;

    // Protected CCS is zero-weighted. RT and the factorized MS2 loss drive the
    // trainable forward clone end-to-end.
    ...
    let losses = multi_task_loss_with_ms2_config(
        &first.base,
        &clean_views.first.targets,
        supervised_weights,
        ms2_loss,
    )?;
    let mut total = losses.total;
    if let Some(rt_robust) = paired_masked_pseudo_huber_v0350(
        &first.base.rt,
        clean_views.first.targets.rt.as_ref(),
        clean_views.first.targets.rt_mask.as_ref(),
        FOUNDATION_RT_ROBUST_DELTA_V0350,
    )? {
        total = (total + rt_robust.affine(FOUNDATION_RT_ROBUST_WEIGHT_V0350, 0.0)?)?;
    }
    ...
    let contextual_ms2_mask = match clean_views.first.targets.ms2_mask.as_ref() {
    ...
    let factorized = foundation_multimodal_ms2_loss_v0350(
        &first,
        clean_views.first.targets.ms2.as_ref(),
    ...
    if let (Some(target), Some(mask)) = (
        ...
        let pearson_loss = masked_pearson_loss_v0350(&first.base.ms2, target, mask, 1.0e-8)?;
        total = (total + pearson_loss.affine(FOUNDATION_MS2_PEARSON_WEIGHT_V0350, 0.0)?)?;
    }

    // Deep supervision attaches directly to the context-conditioned residue
    // representation. This prevents the new encoder from relying exclusively
    // on the terminal fragment decoder to discover cleavage-local structure.
    if let Some(auxiliary) = foundation_fragment_representation_aux_loss_v0350(
        &first,
        clean_views.first.targets.ms2.as_ref(),
        contextual_ms2_mask.as_ref(),
        ms2_loss,
        ...
        total = (total
            + auxiliary
                .total
                .affine(FOUNDATION_FRAGMENT_AUX_WEIGHT_V0350, 0.0)?)?;
    }

    // Separate corrupted views restore representation learning without
    // corrupting the supervised RT/MS2 examples. The trainable clone owns the
    // copied masked-residue/chemistry/contrastive heads, so these losses update
    // the forward representation while the frozen v0.31 anchor remains intact.
    let representation_views =
        representation_collator.collate_views(records, device, seed ^ 0x5245_5052_5630_3335)?;
    let first_rep = model.forward_representation_aux_t(&representation_views.first.input, true)?;
    let second_rep =
        model.forward_representation_aux_t(&representation_views.second.input, true)?;
    let (batch, _) = representation_views.first.input.residue_ids.dims2()?;
    let dummy_rt = Tensor::zeros((batch, 1), DType::F32, device)?;
    let dummy_ccs = Tensor::zeros((batch, 1), DType::F32, device)?;
    let dummy_ms2 = Tensor::zeros((batch, 1, 1), DType::F32, device)?;
    let representation_output = FoundationMultiTaskOutput {
        foundation: first_rep.foundation,
        rt: dummy_rt,
        ccs: dummy_ccs,
        ms2: dummy_ms2,
        residue_logits: first_rep.residue_logits,
        chemistry_reconstruction: first_rep.chemistry_reconstruction,
        contrastive_projection: first_rep.contrastive_projection.clone(),
    };
    let mut representation_targets = representation_views.first.targets.clone();
    representation_targets.rt = None;
    representation_targets.rt_mask = None;
    representation_targets.ccs = None;
    representation_targets.ccs_mask = None;
    representation_targets.ms2 = None;
    representation_targets.ms2_mask = None;
    let representation_weights = FoundationLossWeights {
        rt: 0.0,
        ccs: 0.0,
        ms2: 0.0,
    ...
    let representation_losses = multi_task_loss_with_ms2_config(
        &representation_output,
        &representation_targets,
        representation_weights,
        ms2_loss,
    )?;
    total = (total + representation_losses.total)?;
    let contrastive = contrastive_info_nce_loss(
        &representation_output.contrastive_projection,
        &second_rep.contrastive_projection,
        contrastive_temperature,
    )?;
    total =
        (total + contrastive.affine(FOUNDATION_REPRESENTATION_CONTRASTIVE_WEIGHT_V0350, 0.0)?)?;
    Ok(total)
}
...
fn cross_modal_relation_metrics(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.35 relation objective requires at least two records");
    }
    ...
    let loss = foundation_multimodal_relation_margin_loss_v0350(
        &positive_score,
        &negative_score,
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0350,
    )?;
    ...
    if positive.len() != negative.len() || positive.is_empty() {
        anyhow::bail!("v0.35 relation diagnostics require equal non-empty score batches");
    }
...
fn mass_near_negative_records(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.35 mass-near negatives require at least two records");
    }
    let masses = records
        .iter()
        .map(|record| {
...
fn evaluate_properties(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let fragment_context = FoundationFragmentContextBatchV0350::from_records(
            &owned,
        ...
        let fragment_channel_mask = fragment_context.channel_mask()?;
        let output =
            model.forward_v0350_t(&batch.input, &batch.context, &fragment_context, false)?;
...
fn load_v0310_unchanged_variables(
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.35 VarMap lock poisoned during warm start"))?;
    let mut loaded_variables = 0usize;
    let mut fresh_variables = 0usize;
    let mut missing_reused = Vec::new();
    let mut used_parent_variables = BTreeSet::<String>::new();

    for (name, variable) in data.iter() {
        let parent_name = if name.starts_with("ms2_context_v0350.") {
            // This is the only genuinely new representation component. Its
            // residual output is zero-initialized, so step 0 remains exactly
            // equal to the accepted v0.31 forward model.
            None
        } else if let Some(suffix) = name.strip_prefix("forward_v0350.") {
            Some(suffix.to_string())
        } else if let Some(suffix) = name.strip_prefix("property_refinement_v0350.") {
            Some(format!("property_refinement.{suffix}"))
        } else if let Some(suffix) = name.strip_prefix("fragment_aux_v0350.") {
            Some(format!("fragment_representation_aux.{suffix}"))
        } else {
            Some(name.clone())
        };
        ...
        match tensors.get(&parent_name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.35 warm-start shape mismatch for '{name}' from parent '{parent_name}': parent {:?}, model {:?}",
                        tensor.dims(),
                        variable.as_tensor().dims()
                    );
                }
                variable.set(tensor)?;
                used_parent_variables.insert(parent_name);
                loaded_variables += 1;
            }
            None => missing_reused.push(format!("{name} <- {parent_name}")),
        }
    }
    if !missing_reused.is_empty() {
        anyhow::bail!(
            "frozen v0.31 checkpoint is missing required v0.35 reused variables: {}",
            missing_reused.join(", ")
        );
    }
    let ignored_parent_variables = tensors
        .keys()
        .filter(|name| !used_parent_variables.contains(*name))
        .count();
    drop(data);

    if loaded_variables == 0 || fresh_variables == 0 {
        anyhow::bail!(
            "v0.35 selective warm start is nonfunctional: loaded={loaded_variables} fresh={fresh_variables}"
        );
    }
    Ok(V035WarmStartReport {
        loaded_variables,
        fresh_variables,
...
fn validate_v0350_training_checkpoint_metadata(
    label: &str,
    metadata: &UnifiedPilotMetadata,
    expected_corpus_fingerprint: &str,
    expected_benchmark_fingerprint: &str,
    expected_config: &PeptideFoundationMultimodalV0350Config,
    batch_size: usize,
    seed: u64,
    learning_rate: f64,
    ...
    if metadata.version != 350 {
        anyhow::bail!(
            "v0.35 resume {label} checkpoint has metadata version {}, expected 350",
            metadata.version
        );
    }
    if metadata.objective != "v0350_trainable_forward_representation_context_conditioned_ms2" {
        anyhow::bail!(
            "v0.35 resume {label} checkpoint objective mismatch: {}",
            metadata.objective
        );
    }
    if metadata.corpus_fingerprint != expected_corpus_fingerprint {
        anyhow::bail!(
            "v0.35 resume {label} corpus fingerprint mismatch: checkpoint={} current={expected_corpus_fingerprint}",
            metadata.corpus_fingerprint
        );
    }
    if metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
        anyhow::bail!(
            "v0.35 resume {label} benchmark fingerprint mismatch: checkpoint={} current={expected_benchmark_fingerprint}",
            metadata.benchmark_manifest_fingerprint
        );
    }
    if metadata.v0350_config != *expected_config {
        anyhow::bail!("v0.35 resume {label} architecture config mismatch");
    }
    if metadata.batch_size != batch_size {
        anyhow::bail!(
            "v0.35 resume {label} batch_size mismatch: checkpoint={} requested={batch_size}",
            metadata.batch_size
        );
    }
    if metadata.seed != seed {
        anyhow::bail!(
            "v0.35 resume {label} seed mismatch: checkpoint={} requested={seed}",
            metadata.seed
        );
    }
    ...
    if (metadata.learning_rate - learning_rate).abs() > f64::EPSILON * 32.0 * lr_scale {
        anyhow::bail!(
            "v0.35 resume {label} learning_rate mismatch: checkpoint={} requested={learning_rate}",
            metadata.learning_rate
        );
    }
    if metadata.train_steps != max_total_steps {
        anyhow::bail!(
            "v0.35 resume {label} train-step budget mismatch: checkpoint={} requested={max_total_steps}",
            metadata.train_steps
        );
    }
    if metadata.completed_steps > metadata.train_steps {
        anyhow::bail!(
            "v0.35 resume {label} checkpoint completed_steps={} exceeds train_steps={}",
...
fn read_unified_pilot_metadata(checkpoint: &Path) -> Result<UnifiedPilotMetadata> {
    ...
    serde_yaml::from_str(
        &fs::read_to_string(&metadata_path)
            .with_context(|| format!("failed to read v0.35 metadata {metadata_path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.35 metadata {metadata_path:?}"))
}
...
fn feasible_validation_batches(
    ...
    if batch_size == 0 || requested_batches == 0 {
        anyhow::bail!("v0.35 {label} validation requires positive batch_size/requested_batches");
    }
    let max_batches = requested_batches.min(validation_indices.len() / batch_size);
    if max_batches == 0 {
        anyhow::bail!(
            "v0.35 {label} has only {} records, fewer than batch_size={batch_size}",
    ...
    for &index in validation_indices {
        let source = provenance.get(index).ok_or_else(|| {
            anyhow::anyhow!("v0.35 {label} provenance index {index} is out of bounds")
        })?;
    ...
    for source in available.keys() {
        if !config.validation_source_weights.contains_key(source) {
            anyhow::bail!(
                "v0.35 {label} validation weights are missing represented source '{source}'"
            );
        }
    }
    for source in config.validation_source_weights.keys() {
        if !available.contains_key(source) {
            anyhow::bail!("v0.35 {label} validation weight refers to absent source '{source}'");
        }
    }

    let total_weight: f64 = config.validation_source_weights.values().copied().sum();
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.35 {label} validation source weights have no positive finite mass");
    }

    for batches in (1..=max_batches).rev() {
        let target_records = batches.saturating_mul(batch_size);
        let quotas = weighted_quotas_v0350(
            target_records,
    ...
    anyhow::bail!(
        "v0.35 {label} cannot satisfy validation source weights for even one full batch without replacement"
    )
}

fn weighted_quotas_v0350(
