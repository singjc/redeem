//! Validation-only descendant audit for post-v0.13.9 MS2 consolidation.
//!
//! This compares, on exactly the same source-balanced VALIDATION records:
//! 1. the accepted v0.13.9 final checkpoint carrying the controlled b²/channel-1 reset;
//! 2. a direct descendant continuation checkpoint that must not repeat that reset.
//!
//! The candidate metadata must point directly back to the supplied parent checkpoint, so the
//! b² rescue provenance is verified without rewriting or fabricating checkpoint metadata.
...
fn main() -> Result<()> {
    ...
    if !(4..=7).contains(&args.len()) {
        bail!(
            "usage: foundation_benchmark_ms2_consolidation <training.yaml> <parent_b2_rescue_checkpoint> <candidate_descendant_checkpoint> <output_dir> [max_records_per_source=512] [batch_size=32] [seed=20260912]"
        );
    }

    let training_yaml = PathBuf::from(&args[0]);
    let parent_checkpoint = PathBuf::from(&args[1]);
    ...
    if selected.is_empty() {
        bail!("no VALIDATION records selected for MS2 consolidation audit");
    }

    let parent_metadata = read_unified_metadata(&parent_checkpoint)?;
    let final_metadata = read_unified_metadata(&final_checkpoint)?;
    if parent_metadata.forward_config.ms2_output_activation
        != FoundationMs2OutputActivation::SoftplusV0138
        || final_metadata.forward_config.ms2_output_activation
            != FoundationMs2OutputActivation::SoftplusV0138
    {
        bail!("parent and candidate checkpoints must both use softplus-v0138 MS2 activation");
    }
    if parent_metadata.ms2_head_reset.as_deref() != Some("b2-zero-v0139")
        || parent_metadata.ms2_head_reset_channel != Some(1)
    {
        bail!("parent checkpoint is not marked as the controlled b2-zero-v0139 channel-1 reset");
    }
    if parent_metadata.ms2_head_fingerprint_before_reset.is_none()
        || parent_metadata.ms2_head_fingerprint_after_reset.is_none()
    {
        bail!("parent checkpoint is missing v0.13.9 MS2-head reset fingerprints");
    }
    if !matches!(
        final_metadata.ms2_head_reset.as_deref(),
        None | Some("none")
    ) || final_metadata.ms2_head_reset_channel.is_some()
        || final_metadata.ms2_head_fingerprint_before_reset.is_some()
        || final_metadata.ms2_head_fingerprint_after_reset.is_some()
    {
        bail!("candidate descendant must record ms2_head_reset=none; the b2 reset must not be repeated during consolidation");
    }

    let expected_parent_model = parent_checkpoint.join("model.safetensors");
    let recorded_parent = final_metadata
        .parent_unified_checkpoint
        .as_deref()
        .ok_or_else(|| {
            anyhow::anyhow!("candidate descendant metadata is missing parent_unified_checkpoint")
        })?;
    if Path::new(recorded_parent) != expected_parent_model.as_path() {
        bail!(
            "candidate descendant does not point directly to the supplied repaired parent: recorded {:?}, expected {:?}",
            recorded_parent,
    ...
    if !parent_metadata
        ...
        || parent_metadata.forward_config != final_metadata.forward_config
        || parent_metadata.inverse_config != final_metadata.inverse_config
    {
        bail!("parent and candidate checkpoints do not share one forward/inverse parameterization");
    }
    if final_metadata.completed_steps == 0 {
        bail!("candidate descendant checkpoint must contain trained continuation steps");
    }
    ...
    let collator = FoundationCollator::new(
        parent_metadata.forward_config.clone(),
    ...
    let records_path = output_dir.join("ms2_consolidation_records.tsv");
    let fragments_path = output_dir.join("ms2_consolidation_fragments.tsv");
    let source_summary_path = output_dir.join("ms2_consolidation_source_summary.tsv");
    let family_summary_path = output_dir.join("ms2_consolidation_family_summary.tsv");
    let channel_summary_path = output_dir.join("ms2_consolidation_channel_summary.tsv");
    let head_summary_path = output_dir.join("ms2_consolidation_head_summary.tsv");
    let paired_summary_path = output_dir.join("ms2_consolidation_paired_summary.tsv");
    ...
    let states = [
        ("parent_b2_rescue_v0139", &parent_checkpoint),
        ("final_consolidation_candidate", &final_checkpoint),
    ];
    ...
    let paired = BTreeMap::from([(
        "consolidation_effect".to_string(),
        paired_metrics(
            &evaluations
                .get("parent_b2_rescue_v0139")
                .expect("parent checkpoint evaluation must exist")
                .record_metrics,
            &evaluations
                .get("final_consolidation_candidate")
                .expect("candidate checkpoint evaluation must exist")
                .record_metrics,
        )?,
    )]);
    ...
    println!("candidate_checkpoint\t{}", final_checkpoint.display());
    println!(
        "b2_reset_lineage\talgorithm=b2-zero-v0139\tchannel=1\tfingerprint_before={}\tfingerprint_after={}\tcandidate_reset=none",
        parent_metadata
    ...
    for label in ["parent_b2_rescue_v0139", "final_consolidation_candidate"] {
        ...
        println!(
            "ms2_consolidation_summary\tcheckpoint={label}\tactivation={}\tcompleted_steps={}\trecords={}\tfragments={}\tpointwise_mse={}\tpointwise_mae={}\tcosine={}\tspectral_angle={}\tpearson={}\texact_zero_fraction={}\tnear_zero_1e4_fraction={}\tnear_zero_1e3_fraction={}\tnear_zero_1e2_fraction={}\tmean_target_intensity={}\tmean_predicted_intensity={}",
    ...
    print_paired_summary(
        ...
        paired
            .get("consolidation_effect")
            .expect("paired consolidation comparison must exist"),
    );
