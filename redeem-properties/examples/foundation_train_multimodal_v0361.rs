//! v0.36.1 loss-conditioned CCS-only specialist refinement from frozen v0.35.
//!
...
const V036_VERSION: u32 = 361;
const V036_OBJECTIVE: &str =
    "v0361_frozen_v0350_ccs_only_train_residual_normalized_robust_refinement";
const V036_MAX_STEPS_PER_EPOCH: usize = 1_024;
const V036_MAX_DEV_BATCHES: usize = 32;
const V036_MAX_HOLDOUT_BATCHES: usize = 32;
const V036_LOSS_CALIBRATION_BATCHES: usize = 32;
const V036_CCS_MSE_WEIGHT: f64 = 0.25;
const V036_CCS_ROBUST_WEIGHT: f64 = 1.0;
const V036_MIN_LOSS_SCALE: f64 = 1.0e-3;
const V036_MAX_GRADIENT_NORM: f64 = 1.0;
const V036_MATERIAL_OBJECTIVE: f64 = 0.90;
const V036_MATERIAL_CCS_RATIO: f64 = 0.88;
const V036_RT_INVARIANCE_TOLERANCE: f64 = 5.0e-5;
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0361 RUN_V0260.yaml OUTPUT_DIR PARENT_V0350_CHECKPOINT [max_epochs=6] [batch_size=128] [patience=3] [min_delta=0.002] [seed=202610361] [learning_rate=8e-5] [mode=train|finalize]"
        );
    }
    ...
    let seed = parse_or(&args, 8, 202_610_361u64)?;
    ...
    let holdout_indices: Vec<usize> = benchmark
        ...
        .map(|entry| entry.record_index)
        .collect();
    let ccs_train_indices: Vec<usize> = train_indices
        .iter()
        .copied()
        .filter(|&index| corpus.records[index].ccs.is_some())
        .collect();
    for (label, indices) in [
        ("TRAIN", &train_indices),
        ("CCS_TRAIN", &ccs_train_indices),
        ("DEV", &dev_indices),
    ...
    let steps_per_epoch = (ccs_train_indices.len() / batch_size)
    ...
    let loss_calibration_batches = V036_LOSS_CALIBRATION_BATCHES.min(steps_per_epoch).max(1);
    let mut loss_calibration_sampling = train_sampling.clone();
    loss_calibration_sampling.train_steps_per_epoch = Some(loss_calibration_batches);
    let loss_calibration_plan = sample_foundation_training_indices(
        &corpus.records,
        &corpus.provenance,
        &ccs_train_indices,
        batch_size,
        0,
        seed ^ 0x3610_4c4f_5353_4343,
        true,
        &loss_calibration_sampling,
    )?;
    require_plan_records(
        "ccs_loss_calibration",
        &loss_calibration_plan,
        loss_calibration_batches.saturating_mul(batch_size),
    )?;
    ...
    let ccs_loss_scale_model_space = calibrate_ccs_parent_residual_scale(
        &model,
        &clean_collator,
        &corpus.records,
        &loss_calibration_plan.indices,
        batch_size,
    ...
    println!("v0361_version\tv0.36.1-ccs-only-loss-conditioned-specialist");
    ...
    println!("ccs_train_records\t{}", ccs_train_indices.len());
    ...
    println!(
        ...
        loss_calibration_plan.indices.len()
    );
    println!("ccs_loss_scale_model_space\t{ccs_loss_scale_model_space:.8}");
    println!("ccs_loss_objective\t0.25_scaled_mse_plus_1.0_scaled_pseudo_huber");
    ...
    println!("rt_residual_policy\texact_zero_frozen_v0350_identity");
    ...
    println!("material_dev_gate\tccs_ratio<=0.88_and_rt_invariant_and_ms2_invariant");
    println!("historical_validation_reused_for_v0361_selection\tNO");
    ...
    if finalize_only {
        ...
        validate_rt_invariance(best_dev, best_metadata.initial_dev_metrics)?;
        ...
        println!("v0361_finalize_dev_objective\t{objective:.8}");
        println!("v0361_finalize_rt_ratio\t{rt_ratio:.8}");
        println!("v0361_finalize_ccs_ratio\t{ccs_ratio:.8}");
        if objective > V036_MATERIAL_OBJECTIVE || ccs_ratio > V036_MATERIAL_CCS_RATIO {
            anyhow::bail!("v0.36.1 DEV result does not satisfy the fixed CCS materiality gate; refusing HOLDOUT");
        }
        println!("v0361_finalize_only\ttrue");
        println!(
            "v0361_finalize_best_step\t{}",
        ...
        println!(
            "v0361_finalize_complete\tbest_step={}",
            best_metadata.completed_steps
        );
    ...
    for epoch in 1..=max_epochs {
        ...
        println!("v0361_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
        ...
        for local_step in 0..steps_per_epoch {
            ...
            if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                println!(
                    "v0361_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={loss_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                    local_step + 1,
        ...
        validate_rt_invariance(dev, initial_dev)?;
        ...
        let improved = ccs_ratio < 1.0 && best_objective - objective > min_delta;
        println!(
        ...
        if improved {
            ...
            println!("v0361_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}");
        } else {
            stale_epochs += 1;
        }
        println!("v0361_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}");
        if stale_epochs >= patience {
            stopped_early = true;
            println!("v0361_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}");
            break;
        }
    }
    ...
    println!("historical_validation_reused_for_v0361_selection\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0361_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");

    if best_step == 0 {
        println!("v0361_material_dev_gain\tNO");
        println!("v0361_finalize_required\tNO");
        println!("v0361_rethink_required\tYES");
    } else {
        ...
        validate_rt_invariance(best_dev, initial_dev)?;
        validate_ms2_invariance(best_dev, initial_dev)?;
        let material =
            best_objective <= V036_MATERIAL_OBJECTIVE && ccs_ratio <= V036_MATERIAL_CCS_RATIO;
        println!("v0361_best_rt_ratio\t{rt_ratio:.8}");
        println!("v0361_best_ccs_ratio\t{ccs_ratio:.8}");
        println!(
            "v0361_rt_stretch_target_met\t{}",
            yes_no(
                best_dev
                    .rt_mae_native
        ...
        if material {
            println!("v0361_material_dev_gain\tYES");
            println!("v0361_finalize_required\tYES");
            println!("v0361_rethink_required\tNO");
        } else {
            println!("v0361_material_dev_gain\tNO");
            println!("v0361_finalize_required\tNO");
            println!("v0361_rethink_required\tYES");
...
fn scalar_loss(
    ...
    let (target, mask) = match (batch.targets.ccs.as_ref(), batch.targets.ccs_mask.as_ref()) {
        ...
        _ => anyhow::bail!("v0.36.1 CCS-only training batch contains no CCS labels"),
    };
    let mse = masked_scaled_mse(&output.ccs, target, mask, ccs_loss_scale_model_space)?;
    let robust = masked_scaled_pseudo_huber(
        &output.ccs,
        target,
        mask,
        ccs_loss_scale_model_space,
    ...
    Ok((mse.affine(V036_CCS_MSE_WEIGHT, 0.0)? + robust.affine(V036_CCS_ROBUST_WEIGHT, 0.0)?)?)
}

fn calibrate_ccs_parent_residual_scale(
    model: &PeptideFoundationMultimodalV0360Model,
    collator: &FoundationCollator,
    records: &[FoundationTrainingRecord],
    indices: &[usize],
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let broadcast_mask = mask.broadcast_as(output.ccs.dims())?;
        let squared = (&output.ccs - target)?
            .sqr()?
            .broadcast_mul(&broadcast_mask)?
            .sum_all()?;
        let count = broadcast_mask.sum_all()?;
        squared_error_sum += f64::from(squared.to_scalar::<f32>()?);
        label_count += f64::from(count.to_scalar::<f32>()?);
    }
    if !(label_count > 0.0 && squared_error_sum.is_finite()) {
        anyhow::bail!("v0.36.1 CCS TRAIN loss calibration has no finite labeled residuals");
    }
    Ok((squared_error_sum / label_count)
        .sqrt()
        .max(V036_MIN_LOSS_SCALE))
}
...
fn masked_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.36.1 scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
    if !(scale > 0.0 && scale.is_finite()) {
        anyhow::bail!("v0.36.1 invalid CCS loss scale {scale}");
    }
...
fn masked_scaled_pseudo_huber(
    ...
    if !(scale > 0.0 && scale.is_finite() && delta > 0.0 && delta.is_finite()) {
        anyhow::bail!("v0.36.1 invalid robust loss scale={scale} delta={delta}");
    }
    let mask = mask.broadcast_as(prediction.dims())?;
...
fn validate_rt_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        if (current - initial).abs() > V036_RT_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.36.1 changed protected {label}: baseline={initial:.8} current={current:.8}"
            );
        }
    }
    Ok(())
...
fn validate_metadata(
    ...
    if metadata.version != V036_VERSION || metadata.objective != V036_OBJECTIVE {
        anyhow::bail!("v0.36.1 checkpoint identity mismatch");
    }
    if !(metadata.ccs_loss_scale_model_space > 0.0
        && metadata.ccs_loss_scale_model_space.is_finite())
    {
        anyhow::bail!("v0.36.1 checkpoint has invalid CCS loss scale");
    }
