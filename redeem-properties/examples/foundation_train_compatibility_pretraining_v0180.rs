//! Train the v0.18.0 proposal-hard-negative end-to-end compatibility representation.
//!
//! v0.18.0 keeps the v0.17 end-to-end compatibility architecture and 128-way
//! listwise objective, but closes the mass-only hard-negative lane. TRAIN
//! negatives come only from a TRAIN-partition candidate export generated by the
//! frozen accepted v0.13.23 two-view proposal system. The known TRAIN positive
//! is injected in column 0 and competes against the top 127 non-I/L-equivalent
//! proposal survivors by the frozen legacy rank. TEST is never consumed.
//!
...
//! validation candidate windows, without tuning a legacy-score blend.
...
const PROPOSAL_NEGATIVES_PER_ANCHOR: usize = 127;
...
impl RunMode {
    fn parse(value: Option<&str>) -> Result<Self> {
        match value.unwrap_or("full").trim().to_ascii_lowercase().as_str() {
            "full" => Ok(Self::Full),
            "smoke" => Ok(Self::Smoke),
            other => anyhow::bail!("unsupported v0.18.0 mode {other:?}; expected full or smoke"),
        }
    }
...
struct CompatibilityMetadata {
    ...
    proposal_negatives_per_anchor: usize,
    ...
    train_candidate_groups_input: usize,
    train_spectrum_anchors: usize,
    train_proposal_groups: usize,
    train_positive_exact_present_groups: usize,
    train_positive_il_present_groups: usize,
    proposal_negative_legacy_rank_mean: f64,
    proposal_negative_legacy_rank_max: usize,
...
fn main() -> Result<()> {
    ...
    if args.len() < 6 || args.len() > 7 {
        anyhow::bail!(
            "usage: foundation_train_compatibility_pretraining_v0180 RUN.yaml UNIFIED_CHECKPOINT TRAIN_PROPOSAL_CANDIDATES.tsv VALIDATION_CANDIDATES.tsv OUTPUT_DIR [full|smoke]"
        );
    }
    let training_yaml = PathBuf::from(&args[1]);
    let unified_checkpoint = PathBuf::from(&args[2]);
    let training_path = PathBuf::from(&args[3]);
    ...
    reject_test_path(&training_path)?;
    ...
    let training_candidate_groups = read_candidate_groups(&training_path, true)
        .with_context(|| format!("read TRAIN proposal candidate TSV {training_path:?}"))?;
    if training_candidate_groups.is_empty() {
        anyhow::bail!("v0.18.0 TRAIN proposal candidate TSV contains no mass-valid groups");
    }
    verify_partition(
    ...
    let validation_groups = read_candidate_groups(&validation_path, false)
        .with_context(|| format!("read validation candidate TSV {validation_path:?}"))?;
    if validation_groups.is_empty() {
        anyhow::bail!("v0.18.0 validation candidate TSV contains no mass-valid groups");
    }
    verify_partition(
        &validation_groups,
        &benchmark,
    ...
    if metadata.forward_config.model_dim != 96 || metadata.inverse_config.model_dim != 96 {
        anyhow::bail!(
            "v0.18.0 is anchored to the accepted 96-d unified checkpoint; forward={} inverse={}",
    ...
    if metadata.forward_config.model_dim != metadata.inverse_config.model_dim {
        anyhow::bail!("v0.18.0 requires matching forward/inverse model widths");
    }
    ...
    drop(benchmark);
    let (proposal_groups, proposal_stats) = build_proposal_hard_groups(
        &training_candidate_groups,
        &corpus.records,
    ...
    if proposal_groups.len() < mode.anchor_batch() {
        anyhow::bail!(
            "v0.18.0 proposal-group construction produced only {} groups, fewer than anchor batch {}",
            proposal_groups.len(),
            mode.anchor_batch()
        );
    }
    if mode == RunMode::Full && proposal_groups.len() < MIN_FULL_PROPOSAL_GROUPS {
        anyhow::bail!(
            "v0.18.0 full training requires at least {MIN_FULL_PROPOSAL_GROUPS} exact 128-way TRAIN proposal groups; observed {}. Generate a broader frozen v0.13.23 TRAIN proposal export before scientific validation",
            proposal_groups.len()
        );
    }
    if proposal_groups
        .iter()
        .any(|group| group.negatives.len() != PROPOSAL_NEGATIVES_PER_ANCHOR)
    {
        anyhow::bail!(
            "v0.18.0 requires every retained TRAIN proposal group to contain exactly {PROPOSAL_NEGATIVES_PER_ANCHOR} non-I/L proposal negatives"
        );
    }

    log_host_memory("after_proposal_group_construction");
    ...
    println!(
        "objective\tpositive_first_128way_listwise_frozen_v01323_proposal_hard_negative_training"
    );
    ...
    println!("candidate_generation\tTRAIN_EXPORT_AND_VALIDATION_FROZEN_V01323_NO_TEST");
    ...
    println!("negative_sampling_policy\ttop_127_frozen_v01323_proposal_survivors_by_legacy_rank_excluding_positive_il_no_backfill");
    println!("minimum_full_proposal_groups\t{MIN_FULL_PROPOSAL_GROUPS}");
    println!("training_candidate_tsv\t{}", training_path.display());
    ...
    println!("train_group_width_contract\tEXACT_128_WAY_PROPOSAL_MATCHED");
    println!(
    ...
    let mut window_negative_legacy_rank = 0.0f64;
    let mut window_negative_legacy_rank_max = 0usize;
    ...
    for step0 in 0..mode.train_steps() {
        ...
        window_negative_legacy_rank += batch.mean_selected_negative_legacy_rank;
        window_negative_legacy_rank_max =
            window_negative_legacy_rank_max.max(batch.max_selected_negative_legacy_rank);
        window_steps += 1;

        if step % LOG_EVERY_STEPS == 0 || step == mode.train_steps() {
            println!(
                "compatibility_training\tstep={}\tsteps_total={}\tmean_listwise_loss={:.8}\tmean_train_top1_fraction={:.6}\tmean_gradient_norm={:.6}\tmean_gradient_scale={:.6}\tmean_selected_proposal_legacy_rank={:.3}\tmax_selected_proposal_legacy_rank={}\tunique_anchor_groups_seen={}",
                step,
                mode.train_steps(),
                ...
                window_negative_legacy_rank / window_steps as f64,
                window_negative_legacy_rank_max,
                unique_anchors.len(),
            );
            window_loss = 0.0;
            ...
            window_negative_legacy_rank = 0.0;
            window_negative_legacy_rank_max = 0;
            window_steps = 0;
        }
    }
    ...
    if mode == RunMode::Full {
        println!(
            "v0180_representation_gate\trequired_literal={}\trequired_il={}\trequired_oracle_literal={}\trequired_oracle_il={}\tobserved_literal={}\tobserved_il={}\tobserved_oracle_literal={}\tobserved_oracle_il={}\tgate={}",
        ...
        println!(
            "v0180_stop_rule\t{}",
            if gate {
                "ACCEPT_PROPOSAL_HARD_NEGATIVE_COMPATIBILITY_REPRESENTATION"
            } else {
                "PROPOSAL_HARD_NEGATIVE_REPRESENTATION_GATE_NOT_MET_REASSESS_COMPATIBILITY_OBJECTIVE"
            }
        );
    } else {
        println!("v0180_representation_gate\tNOT_EVALUATED_SMOKE_RUNTIME_ONLY");
        println!("v0180_stop_rule\tSMOKE_RUNTIME_ONLY_NO_SCIENTIFIC_DECISION");
    }
    ...
    let metadata_out = CompatibilityMetadata {
        ...
        objective: "positive_first_128way_listwise_frozen_v01323_proposal_hard_negative_training".into(),
        ...
        candidate_generation: "TRAIN_EXPORT_AND_VALIDATION_FROZEN_V01323_NO_TEST".into(),
        ...
        training_candidate_tsv: training_path.display().to_string(),
        ...
        proposal_negatives_per_anchor: PROPOSAL_NEGATIVES_PER_ANCHOR,
        training_list_width: PROPOSAL_LIST_WIDTH,
        candidate_pairs_per_optimizer_step: mode.anchor_batch() * PROPOSAL_LIST_WIDTH,
        negative_sampling_policy: "top_127_frozen_v01323_proposal_survivors_by_legacy_rank_excluding_positive_il_no_backfill".into(),
        ...
        train_candidate_groups_input: proposal_stats.input_candidate_groups,
        train_spectrum_anchors: proposal_stats.spectrum_anchors,
        train_proposal_groups: proposal_groups.len(),
        train_positive_exact_present_groups: proposal_stats.positive_exact_present_groups,
        train_positive_il_present_groups: proposal_stats.positive_il_present_groups,
        proposal_negative_legacy_rank_mean: proposal_stats.mean_selected_legacy_rank,
        proposal_negative_legacy_rank_max: proposal_stats.max_selected_legacy_rank,
...
fn build_proposal_hard_groups(
    ...
    let mut spectrum_anchors = 0usize;
    let mut positive_exact_present_groups = 0usize;
    let mut positive_il_present_groups = 0usize;
    ...
    for candidate_group in candidate_groups {
        let anchor = records.get(candidate_group.record_index).with_context(|| {
            format!(
                "TRAIN proposal group record {} exceeds corpus",
                candidate_group.record_index
            )
        })?;
        if !training_record_has_usable_spectrum(anchor)
            || !peptide_supported(&anchor.peptidoform, max_sequence_len)
        {
            continue;
        }
        spectrum_anchors += 1;
        ...
        let selected_indices = proposal_negative_indices(&candidate_group.rows, &target_il);
        if selected_indices.len() != PROPOSAL_NEGATIVES_PER_ANCHOR {
            continue;
        }
        ...
        for index in selected_indices {
            ...
            if !peptide_supported(&peptide, max_sequence_len) {
                supported = false;
                break;
            }
            selected_rank_sum += row.legacy_rank as f64;
            selected_rank_count += 1;
            selected_rank_max = selected_rank_max.max(row.legacy_rank);
            negatives.push(row);
        }
        if !supported || negatives.len() != PROPOSAL_NEGATIVES_PER_ANCHOR {
            continue;
        }
        positive_exact_present_groups +=
            usize::from(candidate_group.rows.iter().any(|row| row.exact));
        positive_il_present_groups +=
            usize::from(candidate_group.rows.iter().any(|row| row.il_exact));
    ...
    if groups.is_empty() {
        anyhow::bail!(
            "v0.18.0 found no TRAIN proposal groups with exactly {PROPOSAL_NEGATIVES_PER_ANCHOR} supported non-I/L negatives"
        );
    }
    ...
    Ok((
        ...
        ProposalMiningStats {
            ...
            positive_exact_present_groups,
            positive_il_present_groups,
            mean_selected_legacy_rank: selected_rank_sum / selected_rank_count.max(1) as f64,
            max_selected_legacy_rank: selected_rank_max,
        },
    ))
}
...
fn prune_unused_spectrum_payloads(
    ...
    for _ in 0..mode.train_steps() {
        for group_index in planned_sampler.next_batch(mode.anchor_batch()) {
            let group = groups
                .get(group_index)
                .with_context(|| format!("planned proposal group {group_index} exceeds groups"))?;
...
fn build_training_batch(
    ...
    for &group_index in group_indices {
        let group = groups
            .get(group_index)
            .with_context(|| format!("proposal group index {group_index} exceeds groups"))?;
        let anchor = records
        ...
        if group.negatives.len() != PROPOSAL_NEGATIVES_PER_ANCHOR {
            anyhow::bail!("v0.18.0 internal proposal group width mismatch");
        }
        for candidate in &group.negatives {
            peptides.push(exported_peptidoform(candidate)?);
            selected_rank_sum += candidate.legacy_rank as f64;
            selected_rank_count += 1;
            selected_rank_max = selected_rank_max.max(candidate.legacy_rank);
        }
    ...
    if peptides.len() != group_indices.len() * PROPOSAL_LIST_WIDTH {
        anyhow::bail!("v0.18.0 internal proposal list width mismatch");
    }
...
fn validate_candidate_contract(groups: &[CandidateGroup]) -> Result<ValidationContract> {
    ...
    {
        anyhow::bail!(
            "v0.18.0 frozen candidate oracle/window mismatch: oracle={}/{} window={}/{} expected={}/{}",
    ...
    {
        anyhow::bail!(
            "v0.18.0 frozen legacy baseline mismatch: observed={}/{} expected={}/{}",
...
fn read_candidate_groups(
    ...
    if require_proposal_export {
        for required in [
            ...
            if !index.contains_key(required) {
                anyhow::bail!(
                    "v0.18.0 TRAIN proposal TSV missing frozen-generator provenance column '{required}'"
                );
            }
        }
    }
...
fn verify_partition(
    ...
    for group in groups {
        if !allowed.contains(&group.record_index) {
            anyhow::bail!(
                "v0.18.0 {label} candidate group {} is not assigned to expected benchmark partition",
                group.record_index
            );
        }
    }
...
fn reject_test_path(path: &Path) -> Result<()> {
    ...
    if suspicious {
        anyhow::bail!("v0.18.0 forbids TEST-partition inputs; suspicious path {path:?}");
    }
    Ok(())
}
...
fn exported_peptidoform(row: &CandidateRow) -> Result<PeptidoformInput> {
        ...
        .filter(|part| !part.trim().is_empty())
    {
            ...
            .and_then(|value| value.strip_suffix(')'))
        {
            ...
            anyhow::bail!(
                "v0.18.0 cannot reconstruct unsupported exported modification site '{site}'"
            );
        }
    }
    nterm.sort_unstable();
...
fn require_nonzero_end_to_end_gradients(
    ...
    for (label, prefix) in [
        ...
        if !(norm > 0.0 && norm.is_finite()) {
            anyhow::bail!(
                "v0.18.0 end-to-end gradient probe failed for {label}: gradient_norm={norm}"
            );
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn proposal_hard_negative_protocol_is_exactly_128_way() {
        assert_eq!(PROPOSAL_NEGATIVES_PER_ANCHOR, 127);
        assert_eq!(PROPOSAL_LIST_WIDTH, VALIDATION_INTERACTION_WINDOW);
        assert_eq!(RunMode::Full.anchor_batch() * PROPOSAL_LIST_WIDTH, 1024);
    }
    ...
    #[test]
    fn proposal_negative_selection_excludes_il_and_preserves_legacy_order() {
        let rows = vec![
            CandidateRow {
                sequence: "PEPTIDEI".into(),
        ...
        let selected = proposal_negative_indices(&rows, &il_sequence_key("PEPTIDEI"));
