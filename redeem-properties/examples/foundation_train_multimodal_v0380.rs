//! v0.38 mobility-native trainable CCS representation from frozen v0.35.
//!
...
//! v0.35 peptide representation and predicts a zero-initialized native ion-mobility
//! residual. TRAIN-only mobility consensus/reliability supervision is used for
...
const V038_VERSION: u32 = 380;
const V038_OBJECTIVE: &str = "v0380_mobility_native_trainable_ccs_representation";
const V038_ARCHITECTURE: &str = "v0.38-frozen-v0350-mobility-native-trainable-ccs-representation";
...
fn main() -> Result<()> {
    ...
    if max_epochs == 0 || batch_size < 2 || patience == 0 {
        anyhow::bail!("v0.38 requires max_epochs>0, batch_size>=2, and patience>0");
    }
    if !(min_delta > 0.0 && min_delta.is_finite()) {
        anyhow::bail!("v0.38 min_delta must be positive and finite");
    }
        ...
        for name in ["initial", "best"] {
            ...
            for file in [
                ...
                if !directory.join(file).is_file() {
                    anyhow::bail!("v0.38 finalize is missing {:?}", directory.join(file));
                }
            }
        }
        if output_root.join("final").exists() {
            anyhow::bail!(
                "v0.38 final checkpoint already exists; HOLDOUT must not be consumed twice"
            );
        }
    } else if output_root.exists() {
        anyhow::bail!(
            "v0.38 TRAIN output directory already exists: {:?}",
            output_root
        );
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.38 requires a CUDA device")?;
    ...
    if parent_metadata.version != 350 {
        anyhow::bail!(
            "v0.38 requires v0.35 metadata version 350, observed {}",
    ...
    {
        anyhow::bail!(
            "v0.38 requires the accepted v0.35 objective, observed {}",
            parent_metadata.objective
        );
    }
    if parent_metadata.completed_steps == 0 {
        anyhow::bail!("v0.38 refuses an unselected v0.35 baseline checkpoint");
    }
    ...
    if parent_metadata.corpus_fingerprint != current_corpus_fingerprint {
        anyhow::bail!("v0.38 corpus fingerprint differs from frozen v0.35 parent");
    }
    if parent_metadata.benchmark_manifest_fingerprint != current_benchmark_fingerprint {
        anyhow::bail!("v0.38 benchmark fingerprint differs from frozen v0.35 parent");
    }
        ...
        .chain(&holdout_indices)
    {
        ...
        if length > forward_config.max_sequence_len {
            anyhow::bail!(
                "v0.38 record {index} length {length} exceeds max_sequence_len={}",
    ...
    println!("v0380_version\tv0.38-mobility-native-trainable-ccs-representation");
    ...
    println!("historical_validation_reused_for_v0380_selection\tNO");
    ...
    if finalize_only {
        ...
        if best_metadata.completed_steps == 0 {
            anyhow::bail!("v0.38 has no DEV improvement; refusing to consume HOLDOUT");
        }
        ...
        println!("v0380_finalize_dev_objective\t{objective:.8}");
        println!("v0380_finalize_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0380_finalize_consensus_ccs_ratio\t{consensus_ratio:.8}");
        if raw_ratio > V038_MATERIAL_RAW_CCS_RATIO
            || consensus_ratio > V038_MATERIAL_CONSENSUS_CCS_RATIO
        {
            anyhow::bail!(
                "v0.38 DEV result does not satisfy the fixed materiality gate; refusing HOLDOUT"
            );
        }

        println!("v0380_finalize_only\ttrue");
        println!(
            "v0380_finalize_best_step\t{}",
        ...
        println!(
            "v0380_holdout_raw_ccs\trecords={}\tmae={holdout_raw_ccs:.8}",
            holdout_ccs_indices.len()
        );
        println!(
            "v0380_holdout_consensus_ccs\tidentities={}\tmae={holdout_consensus_ccs:.8}",
        ...
        println!(
            "v0380_finalize_complete\tbest_step={}",
            best_metadata.completed_steps
        );
    ...
    for epoch in 1..=max_epochs {
        ...
        println!(
            "v0380_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}\texamples={needed}"
        );
        ...
        for local_step in 0..steps_per_epoch {
            ...
            if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                println!(
                    "v0380_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={loss_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                    local_step + 1,
        ...
        if improved {
            ...
            println!("v0380_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}\traw_ccs_ratio={raw_ratio:.8}\tconsensus_ccs_ratio={consensus_ratio:.8}");
        } else {
            stale_epochs += 1;
        }
        println!("v0380_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}");
        if stale_epochs >= patience {
            stopped_early = true;
            println!("v0380_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}");
            break;
        }
    }
    ...
    println!("historical_validation_reused_for_v0380_selection\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0380_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");

    if best_step == 0 {
        println!("v0380_material_dev_gain\tNO");
        println!("v0380_finalize_required\tNO");
        println!("v0380_rethink_required\tYES");
    } else {
        ...
        let material = raw_ratio <= V038_MATERIAL_RAW_CCS_RATIO
            && consensus_ratio <= V038_MATERIAL_CONSENSUS_CCS_RATIO;
        println!("v0380_best_raw_dev_ccs_mae\t{best_raw_dev:.8}");
        println!("v0380_best_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0380_best_consensus_dev_ccs_mae\t{best_consensus_dev:.8}");
        println!("v0380_best_consensus_ccs_ratio\t{consensus_ratio:.8}");
        println!(
            "v0380_rt_stretch_target_met\t{}",
            yes_no(
                best_protected_dev
                    .rt_mae_native
        ...
        println!(
            "v0380_ccs_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_TARGET_MAE_V0360)
        );
        println!(
            "v0380_ccs_stretch_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_STRETCH_TARGET_MAE_V0360)
        );
        if material {
            println!("v0380_material_dev_gain\tYES");
            println!("v0380_finalize_required\tYES");
            println!("v0380_rethink_required\tNO");
        } else {
            println!("v0380_material_dev_gain\tNO");
            println!("v0380_finalize_required\tNO");
            println!("v0380_rethink_required\tYES");
    ...
    let owned = examples
        ...
        .map(|example| {
            records
                ...
                .cloned()
                .ok_or_else(|| anyhow::anyhow!("v0.38 representative index outside corpus"))
        })
...
fn calibrate_mobility_residual_scales(
    ...
    if !(weight_sum > 0.0 && mobility_squared.is_finite() && ccs_squared.is_finite()) {
        anyhow::bail!("v0.38 TRAIN loss calibration has no finite weighted labels");
    }
...
fn evaluate_raw_ccs_indices(
    ...
    if count == 0 {
        anyhow::bail!("v0.38 raw CCS evaluation contains no labels");
    }
...
fn evaluate_consensus_targets(
    ...
    if count == 0 {
        anyhow::bail!("v0.38 consensus CCS evaluation contains no labels");
    }
    Ok(absolute_error / count as f64)
}
...
fn masked_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.38 scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
    if !(scale > 0.0 && scale.is_finite()) {
        anyhow::bail!("v0.38 invalid CCS loss scale {scale}");
    }
...
fn validate_rt_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        if (current - initial).abs() > V038_RT_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.38 changed frozen {label}: baseline={initial:.8} current={current:.8}"
            );
        }
    }
    Ok(())
...
fn load_v0350_parent_variables(
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.38 VarMap lock poisoned during warm start"))?;
        ...
        match tensors.get(&parent_name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.38 warm-start shape mismatch for {name} <- {parent_name}: parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing.is_empty() {
        anyhow::bail!(
            "v0.35 parent is missing required v0.38 reused variables: {}",
            missing.join(", ")
        );
    }
    ...
    drop(data);
    if loaded == 0 || fresh == 0 {
        anyhow::bail!("v0.38 warm start is nonfunctional: loaded={loaded} fresh={fresh}");
    }
    Ok(WarmStartReport {
...
fn validate_metadata(
    ...
    if metadata.version != V038_VERSION || metadata.objective != V038_OBJECTIVE {
        anyhow::bail!("v0.38 checkpoint identity mismatch");
    }
    ...
    {
        anyhow::bail!("v0.38 checkpoint has invalid mobility/CCS loss scale");
    }
    ...
    {
        anyhow::bail!("v0.38 checkpoint data/supervision provenance mismatch");
    }
    if metadata.v0380_config != *config {
        anyhow::bail!("v0.38 checkpoint architecture mismatch");
    }
    if metadata.batch_size != batch_size
        || metadata.seed != seed
        || metadata.train_steps != train_steps
    {
        anyhow::bail!("v0.38 checkpoint run-contract mismatch");
    }
    if (metadata.learning_rate - learning_rate).abs()
        > f64::EPSILON * 64.0 * learning_rate.abs().max(1.0)
    {
        anyhow::bail!("v0.38 checkpoint learning-rate mismatch");
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn v0380_affine_fit_recovers_linear_map() {
    ...
    #[test]
    fn v0380_consensus_prefers_reliable_source() {
    ...
    #[test]
    fn v0380_overlapping_views_collapse_to_project_families() {
    ...
    #[test]
    fn v0380_epoch_order_is_deterministic_and_unique_within_cycle() {
        let examples = (0..32)
