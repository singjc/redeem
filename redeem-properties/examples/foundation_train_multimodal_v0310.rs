//! v0.31 protected-CCS fragment-aware property-view continuation from frozen v0.27.
//!
//! This is one predeclared architecture experiment, not a hyperparameter sweep.
//! v0.30 established a reproducible RT/MS2 gain but regressed CCS on TRAIN-HOLDOUT.
//! v0.31 therefore freezes the accepted v0.27 shared encoder + CCS path and learns
//! a zero-initialized residue-level fragment-aware property refinement used only by
//! RT/MS2/self-supervision. Inverse objectives are evaluated as diagnostics but are
//! not optimized in this lane. Historical VALIDATION and TEST remain closed.
...
use redeem_properties::foundation::{
    ...
    foundation_fragment_representation_aux_loss_v0310, foundation_ms2_loss,
    foundation_multimodal_ms2_loss_v0310, foundation_multimodal_relation_margin_loss_v0310,
    ...
    FoundationDiffusionConfig, FoundationDiffusionVocabulary, FoundationFragmentContextBatchV0310,
    ...
    PeptideFoundationMultimodalV0270Config, PeptideFoundationMultimodalV0310Config,
    PeptideFoundationMultimodalV0310Model, PeptidoformInput, PrecursorContextBatch,
    ...
    FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0310, FOUNDATION_MS2_SOFTPLUS_BETA_V0138,
    FOUNDATION_MULTIMODAL_ARCHITECTURE_V0310, FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0310,
    FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0310, FOUNDATION_OPEN_PTM_MASS_SCALE_DA,
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0310 RUN_V0260.yaml OUTPUT_DIR PARENT_V0270_CHECKPOINT [max_epochs=12] [batch_size=32] [patience=4] [min_delta=0.003] [seed=20261031] [learning_rate=2.5e-5] [mode=train|resume|finalize]"
        );
    }
    ...
    let max_epochs = parse_or(&args, 4, 12usize)?;
    let batch_size = parse_or(&args, 5, 32usize)?;
    let patience = parse_or(&args, 6, 4usize)?;
    let min_delta = parse_or(&args, 7, 0.003f64)?;
    let seed = parse_or(&args, 8, 20_261_031u64)?;
    let learning_rate = parse_or(&args, 9, 2.5e-5f64)?;
    ...
    let (resume_training, finalize_only) = match run_mode {
        ...
        "finalize" => (false, true),
        other => anyhow::bail!(
            "unsupported v0.31 run mode {other:?}; expected train, resume, or finalize"
        ),
    };
    ...
    if finalize_only || resume_training {
        if !output_root.is_dir() {
            anyhow::bail!(
                "v0.31 {run_mode} mode requires an existing model output directory: {:?}",
                output_root
            );
        }
        ...
        for checkpoint in required_checkpoints {
            for name in [
                ...
                if !path.is_file() {
                    anyhow::bail!("v0.31 {run_mode} mode is missing checkpoint file: {path:?}");
                }
            }
        }
    } else if output_root.exists() {
        anyhow::bail!("v0.31 output directory already exists: {:?}", output_root);
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.31 requires a CUDA device")?;
    ...
    if parent_metadata.version != 270 {
        anyhow::bail!(
            "v0.31 requires the frozen v0.27 parent metadata version 270, observed {}",
            parent_metadata.version
        );
    }
    if parent_metadata.objective != "v0270_rt_specialist_contextual_fragment_tokens_openptm32" {
        anyhow::bail!(
            "v0.31 requires the frozen v0.27 objective, observed {}",
            parent_metadata.objective
        );
    }
    // v0.31 changes only the property view of the accepted v0.27 shared
    // representation. Preserve the entire v0.27 architecture exactly.
    ...
    let expected_v0270 = PeptideFoundationMultimodalV0270Config::fixed(
        forward_config.clone(),
        inverse_config.clone(),
    )?;
    if parent_metadata.v0270_config != expected_v0270 {
        anyhow::bail!("v0.31 parent v0.27 architecture config does not match its base configs");
    }
    let v0310_config =
        PeptideFoundationMultimodalV0310Config::fixed(parent_metadata.v0270_config.clone())?;

    // The v0.31 prepare step excludes peptides that cannot be represented by
    ...
    if let Some((record_index, sequence, sequence_len)) = first_overlength {
        anyhow::bail!(
            "v0.31 prepared benchmark contains {prepared_overlength_records} peptide(s) longer than architecture max_sequence_len={}; first record_index={record_index} length={sequence_len} sequence={sequence}",
    ...
    let model = PeptideFoundationMultimodalV0310Model::new(v0310_config.clone(), vb)?;

    // Restore every v0.27 namespace exactly. Only property_refinement.* and
    // fragment_representation_aux.* are fresh. The property refinement has a
    // zero-initialized residual output, so all accepted v0.27 property predictions
    // reproduce the frozen parent exactly at step 0.
    let warm_start_report = load_v0270_unchanged_variables(
        &varmap,
        &parent_checkpoint.join("model.safetensors"),
        &device,
    )?;
    let warm_start_description = format!(
        "v0270_all_namespaces_plus_fresh_property_refinement_and_fragment_aux_from={}",
        parent_checkpoint.display()
    );
    let alignment_initialization = "v0270_frozen_checkpoint".to_string();
    ...
    // v0.31 is intentionally a forward-only representation experiment. The
    // frozen v0.27 inverse stack is evaluated but receives no optimizer steps.
    ...
    println!("v0310_version\tv0.31-protected-ccs-fragment-aware-property-view");
    println!("objective\tv0310_protected_ccs_fragment_aware_property_view_v027_heads_openptm32");
    println!("architecture\t{}", FOUNDATION_MULTIMODAL_ARCHITECTURE_V0310);
    ...
    println!("parent_checkpoint_weights_loaded\ttrue_all_v0270_namespaces");
    println!("random_initialization\tproperty_refinement_plus_fragment_representation_aux_only");
    println!(
    ...
    println!(
        "relation_margin\t{}",
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0310
    );
    println!(
        "property_adapter_hidden\t{}",
        FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0310
    );
    println!("fragment_representation_aux_mode\tdirect_adjacent_shared_residue_to_ms2_v1");
    println!(
        "fragment_representation_aux_hidden\t{}",
        FOUNDATION_FRAGMENT_REPRESENTATION_AUX_HIDDEN_V0310
    );
    println!(
        "fragment_representation_aux_weight\t{}",
        FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0310
    );
    println!("fragment_representation_aux_context\tcharge_presence_nce_presence");
    println!("accepted_property_heads\tv0270_heads_warm_started_rt_ms2_trainable_ccs_frozen");
    println!("base_encoder_update_policy\tfrozen_v0270_stop_gradient");
    println!("ccs_update_policy\tfrozen_v0270_encoder_plus_ccs_adapter");
    println!("property_view\th_v0270_detached_plus_zero_initialized_residue_transformer_delta");
    println!("training_objectives\tforward_only_rt_ms2_self_supervision_fragment_aux");
    println!("dev_selection\tboth_rt_and_ms2_must_improve_then_equal_weight_ratio");
    println!("dev_materiality_gate\tobjective_le_0.980_and_protected_ccs_invariant");
    println!("ccs_physics_path\tpreserved_exactly_from_v0270");
    ...
    let metadata = |completed_steps| UnifiedPilotMetadata {
        version: 310,
        objective: "v0310_protected_ccs_fragment_aware_property_view_v027_heads_openptm32".into(),
        ...
        relation_margin: FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0310,
        property_adapter_hidden: FOUNDATION_MULTIMODAL_PROPERTY_HIDDEN_V0310,
        ...
        v0310_config: v0310_config.clone(),
    ...
    if finalize_only {
        ...
        if best_metadata.version != 310 {
            anyhow::bail!(
                "v0.31 finalize mode expected checkpoint metadata version 310, observed {}",
                best_metadata.version
            );
        }
        if best_metadata.completed_steps == 0 {
            anyhow::bail!(
                "v0.31 has no DEV improvement over frozen v0.27; refusing to consume HOLDOUT/finalize baseline"
            );
        }
        if best_metadata.corpus_fingerprint != expected_corpus_fingerprint {
            anyhow::bail!(
                "v0.31 finalize corpus fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
            anyhow::bail!(
                "v0.31 finalize benchmark fingerprint mismatch: checkpoint={} current={}",
        ...
        if best_metadata.batch_size != batch_size {
            anyhow::bail!(
                "v0.31 finalize batch_size mismatch: checkpoint={} requested={batch_size}",
                best_metadata.batch_size
            );
        }
        if best_metadata.seed != seed {
            anyhow::bail!(
                "v0.31 finalize seed mismatch: checkpoint={} requested={seed}",
                best_metadata.seed
            );
        }
        ...
        if !initial_model.is_file() {
            anyhow::bail!("v0.31 finalize requires the saved step-0 initial checkpoint");
        }
        varmap
            .load(&initial_model)
            .with_context(|| format!("failed to restore v0.31 initial model {initial_model:?}"))?;
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore best v0.31 model {best_model:?}"))?;
        ...
        println!("v0310_finalize_dev_objective\t{best_dev_objective:.8}");
        if best_dev_objective > 0.980 {
            anyhow::bail!(
                "v0.31 DEV gain is below the fixed materiality gate (objective={best_dev_objective:.8} > 0.980); refusing to consume HOLDOUT"
            );
        }
        ...
        println!("v0310_finalize_only\ttrue");
        println!("v0310_finalize_source_checkpoint\t{}", best_dir.display());
        println!("v0310_finalize_best_step\t{best_step}");
        println!(
            "v0310_finalize_best_epoch\t{}",
            best_epoch
        ...
        println!("v0310_finalize_complete\tbest_step={best_step}");
    ...
    let (
        ...
        for (label, checkpoint_metadata) in [
            ...
            validate_v0310_training_checkpoint_metadata(
                label,
                checkpoint_metadata,
                &expected_corpus_fingerprint,
                &expected_benchmark_fingerprint,
        ...
        if initial_metadata.completed_steps != 0 {
            anyhow::bail!(
                "v0.31 resume expected initial checkpoint at step 0, observed {}",
        ...
        {
            anyhow::bail!(
                "v0.31 resume requires latest checkpoint at a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                latest_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps % steps_per_epoch != 0 {
            anyhow::bail!(
                "v0.31 resume requires best checkpoint at baseline or a complete epoch boundary; completed_steps={} steps_per_epoch={steps_per_epoch}",
                best_metadata.completed_steps
            );
        }
        if best_metadata.completed_steps > latest_metadata.completed_steps {
            anyhow::bail!(
                "v0.31 resume best checkpoint step {} is newer than latest step {}",
                best_metadata.completed_steps,
                latest_metadata.completed_steps
            );
        }
        ...
        varmap
            .load(&best_model)
            .with_context(|| format!("failed to restore v0.31 best model {best_model:?}"))?;
        ...
        varmap
            .load(&latest_model)
            .with_context(|| format!("failed to restore v0.31 latest model {latest_model:?}"))?;
        let latest_optimizer = latest_dir.join("optimizer.safetensors");
        optimizer
            .load_safetensors(&latest_optimizer)
            .with_context(|| {
                format!("failed to restore v0.31 optimizer state {latest_optimizer:?}")
            })?;
        ...
        if stale_epochs >= patience {
            println!(
                "v0310_resume_already_early_stopped\tcompleted_epoch={completed_epoch}\tbest_epoch={best_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
            );
        }
        println!(
            "v0310_resume\tlatest_epoch={completed_epoch}\tlatest_step={global_step}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstale_epochs={stale_epochs}"
        );
        (
            initial_metrics,
            global_step,
        ...
        // v0.31 is an identity-preserving continuation at the accepted output
        // surface. Seed model selection from the frozen v0.27 parent so a
    ...
    if start_epoch > max_epochs {
        println!(
            "v0310_training_budget_complete\tstart_epoch={start_epoch}\tmax_epochs={max_epochs}\tlatest_step={global_step}"
        );
    } else if stopped_early {
        println!(
            "v0310_training_already_early_stopped\tstart_epoch={start_epoch}\tstale_epochs={stale_epochs}\tpatience={patience}"
        );
    } else {
        for epoch in start_epoch..=max_epochs {
            ...
            println!("v0310_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}");
            ...
            for local_step in 0..steps_per_epoch {
                ...
                if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                    println!(
                        "v0310_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={total_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                        local_step + 1,
            ...
            if improved {
                ...
                println!(
                    "v0310_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}"
                );
            } else {
                stale_epochs += 1;
            }
            ...
            println!(
                "v0310_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}"
            );
            if stale_epochs >= patience {
                stopped_early = true;
                println!(
                    "v0310_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}"
                );
                break;
            }
        }
    ...
    println!("historical_validation_consumed\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0310_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");
    if best_step == 0 {
        println!("v0310_no_dev_improvement_over_parent\tYES");
        println!("v0310_material_dev_gain\tNO");
        println!("v0310_finalize_required\tNO");
        println!("v0310_rethink_required\tYES");
    } else if best_objective <= 0.980 {
        println!("v0310_no_dev_improvement_over_parent\tNO");
        println!("v0310_material_dev_gain\tYES");
        println!("v0310_finalize_required\tYES");
        println!("v0310_rethink_required\tNO");
    } else {
        println!("v0310_no_dev_improvement_over_parent\tNO");
        println!("v0310_material_dev_gain\tNO");
        println!("v0310_finalize_required\tNO");
...
fn normalized_dev_objective(
    ...
    let (p, _i) = metrics;
    let (bp, _bi) = baseline;
    // v0.31 selects only the two intended weak forward properties. CCS is a
    // protected invariant rather than an optimization term: any measurable
    // drift means the architectural isolation failed and the run must stop.
    ...
    if (ccs - 1.0).abs() > 2.0e-4 {
        anyhow::bail!("v0.31 protected CCS drifted from the frozen step-0 path: ratio={ccs:.8}");
    }
    // A gain in one weak property cannot buy a regression in the other. This
    // keeps checkpoint selection aligned with the scientific goal rather than
    // recreating another compensating multi-task trade-off.
    if rt >= 1.0 || ms2 >= 1.0 {
        return Ok(1.0 + 0.50 * (rt - 1.0).max(0.0) + 0.50 * (ms2 - 1.0).max(0.0));
    }
    Ok(0.50 * rt + 0.50 * ms2)
}
...
fn finite_ratio(value: Option<f64>, baseline: Option<f64>, label: &str) -> Result<f64> {
    match (value, baseline) {
        ...
        _ => anyhow::bail!("v0.31 dev objective is missing {label} metric"),
    }
}
...
fn finite_ratio_value(value: f64, baseline: f64, label: &str) -> Result<f64> {
    if !value.is_finite() || !baseline.is_finite() || baseline.abs() <= 1.0e-12 {
        anyhow::bail!(
            "v0.31 dev objective has invalid {label} ratio: value={value}, baseline={baseline}"
        );
    }
    Ok(value / baseline)
}
...
fn copy_checkpoint_dir(source: &Path, destination: &Path) -> Result<()> {
    ...
    for name in [
        ...
        fs::copy(source.join(name), destination.join(name)).with_context(|| {
            format!("failed to copy v0.31 checkpoint file {name} from {source:?}")
        })?;
    }
    Ok(())
}
...
fn forward_loss(
    model: &PeptideFoundationMultimodalV0310Model,
    ...
    let fragment_context =
        FoundationFragmentContextBatchV0310::from_records(records, model.forward_config(), device)?;
    let fragment_channel_mask = fragment_context.channel_mask()?;
    let first = model.forward_v0310_t_with_shared_gradient_scales(
        &views.first.input,
        &views.first.context,
        &fragment_context,
        true,
    ...
    let second_projection = model.peptide_projection_t(&views.second.input, true)?;
    ...
    let factorized = foundation_multimodal_ms2_loss_v0310(
        &first,
    ...
    if let Some(auxiliary) = foundation_fragment_representation_aux_loss_v0310(
        &first,
        views.first.targets.ms2.as_ref(),
        contextual_ms2_mask.as_ref(),
        ms2_loss,
        ...
        total = (total
            + auxiliary
                .total
                .affine(FOUNDATION_FRAGMENT_REPRESENTATION_AUX_WEIGHT_V0310, 0.0)?)?;
    }
...
fn cross_modal_relation_metrics(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.31 relation objective requires at least two records");
    }
    ...
    let loss = foundation_multimodal_relation_margin_loss_v0310(
        &positive_score,
        &negative_score,
        FOUNDATION_MULTIMODAL_RELATION_MARGIN_V0310,
    )?;
    ...
    if positive.len() != negative.len() || positive.is_empty() {
        anyhow::bail!("v0.31 relation diagnostics require equal non-empty score batches");
    }
...
fn mass_near_negative_records(
    ...
    if records.len() < 2 {
        anyhow::bail!("v0.31 mass-near negatives require at least two records");
    }
    let masses = records
        .iter()
        .map(|record| {
...
fn evaluate_properties(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let fragment_context = FoundationFragmentContextBatchV0310::from_records(
            &owned,
        ...
        let fragment_channel_mask = fragment_context.channel_mask()?;
        let output =
            model.forward_v0310_t(&batch.input, &batch.context, &fragment_context, false)?;
...
fn load_v0270_unchanged_variables(
    varmap: &VarMap,
    checkpoint: &Path,
    device: &Device,
) -> Result<V031WarmStartReport> {
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.31 VarMap lock poisoned during warm start"))?;
    let mut loaded_variables = 0usize;
    let mut fresh_variables = 0usize;
    let mut missing_reused = Vec::new();

    for (name, variable) in data.iter() {
        if name.starts_with("property_refinement.")
            || name.starts_with("fragment_representation_aux.")
        {
            fresh_variables += 1;
            continue;
        }
        match tensors.get(name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.31 warm-start shape mismatch for '{name}': parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing_reused.is_empty() {
        anyhow::bail!(
            "frozen v0.27 checkpoint is missing required v0.31 reused variables: {}",
            missing_reused.join(", ")
        );
    }
    ...
    if loaded_variables == 0 || fresh_variables == 0 {
        anyhow::bail!(
            "v0.31 selective warm start is nonfunctional: loaded={loaded_variables} fresh={fresh_variables}"
        );
    }
    Ok(V031WarmStartReport {
        loaded_variables,
        fresh_variables,
...
fn validate_v0310_training_checkpoint_metadata(
    label: &str,
    metadata: &UnifiedPilotMetadata,
    expected_corpus_fingerprint: &str,
    expected_benchmark_fingerprint: &str,
    expected_config: &PeptideFoundationMultimodalV0310Config,
    batch_size: usize,
    seed: u64,
    learning_rate: f64,
    max_total_steps: usize,
) -> Result<()> {
    if metadata.version != 310 {
        anyhow::bail!(
            "v0.31 resume {label} checkpoint has metadata version {}, expected 310",
            metadata.version
        );
    }
    if metadata.objective != "v0310_protected_ccs_fragment_aware_property_view_v027_heads_openptm32"
    {
        anyhow::bail!(
            "v0.31 resume {label} checkpoint objective mismatch: {}",
            metadata.objective
        );
    }
    if metadata.corpus_fingerprint != expected_corpus_fingerprint {
        anyhow::bail!(
            "v0.31 resume {label} corpus fingerprint mismatch: checkpoint={} current={expected_corpus_fingerprint}",
            metadata.corpus_fingerprint
        );
    }
    if metadata.benchmark_manifest_fingerprint != expected_benchmark_fingerprint {
        anyhow::bail!(
            "v0.31 resume {label} benchmark fingerprint mismatch: checkpoint={} current={expected_benchmark_fingerprint}",
            metadata.benchmark_manifest_fingerprint
        );
    }
    if metadata.v0310_config != *expected_config {
        anyhow::bail!("v0.31 resume {label} architecture config mismatch");
    }
    if metadata.batch_size != batch_size {
        anyhow::bail!(
            "v0.31 resume {label} batch_size mismatch: checkpoint={} requested={batch_size}",
            metadata.batch_size
        );
    }
    if metadata.seed != seed {
        anyhow::bail!(
            "v0.31 resume {label} seed mismatch: checkpoint={} requested={seed}",
            metadata.seed
        );
    }
    ...
    if (metadata.learning_rate - learning_rate).abs() > f64::EPSILON * 32.0 * lr_scale {
        anyhow::bail!(
            "v0.31 resume {label} learning_rate mismatch: checkpoint={} requested={learning_rate}",
            metadata.learning_rate
        );
    }
    if metadata.train_steps != max_total_steps {
        anyhow::bail!(
            "v0.31 resume {label} train-step budget mismatch: checkpoint={} requested={max_total_steps}",
            metadata.train_steps
        );
    }
    if metadata.completed_steps > metadata.train_steps {
        anyhow::bail!(
            "v0.31 resume {label} checkpoint completed_steps={} exceeds train_steps={}",
...
fn read_unified_pilot_metadata(checkpoint: &Path) -> Result<UnifiedPilotMetadata> {
    ...
    serde_yaml::from_str(
        &fs::read_to_string(&metadata_path)
            .with_context(|| format!("failed to read v0.31 metadata {metadata_path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.31 metadata {metadata_path:?}"))
}
...
fn feasible_validation_batches(
    ...
    if batch_size == 0 || requested_batches == 0 {
        anyhow::bail!("v0.31 {label} validation requires positive batch_size/requested_batches");
    }
    let max_batches = requested_batches.min(validation_indices.len() / batch_size);
    if max_batches == 0 {
        anyhow::bail!(
            "v0.31 {label} has only {} records, fewer than batch_size={batch_size}",
    ...
    for &index in validation_indices {
        let source = provenance.get(index).ok_or_else(|| {
            anyhow::anyhow!("v0.31 {label} provenance index {index} is out of bounds")
        })?;
    ...
    for source in available.keys() {
        if !config.validation_source_weights.contains_key(source) {
            anyhow::bail!(
                "v0.31 {label} validation weights are missing represented source '{source}'"
            );
        }
    }
    for source in config.validation_source_weights.keys() {
        if !available.contains_key(source) {
            anyhow::bail!("v0.31 {label} validation weight refers to absent source '{source}'");
        }
    }

    let total_weight: f64 = config.validation_source_weights.values().copied().sum();
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.31 {label} validation source weights have no positive finite mass");
    }

    for batches in (1..=max_batches).rev() {
        let target_records = batches.saturating_mul(batch_size);
        let quotas = weighted_quotas_v0310(
            target_records,
    ...
    anyhow::bail!(
        "v0.31 {label} cannot satisfy validation source weights for even one full batch without replacement"
    )
}

fn weighted_quotas_v0310(
