//! v0.38.1 standardized mobility-residual conditioning repair from frozen v0.35.
//!
//! v0.35 is treated as an immutable forward anchor. This executable trains
//! RT, MS2, and inverse remain frozen. CCS owns a trainable clone of the accepted
//! v0.35 peptide representation and predicts a zero-initialized dimensionless
//! standardized ion-mobility residual. TRAIN-only mobility consensus/reliability supervision is used for
//! optimization; DEV selects checkpoints. HOLDOUT is touched only by explicit
//! `finalize` mode after the material DEV gate.
...
use redeem_properties::foundation::{
    ...
    PeptideFoundationMultimodalV0350Config, PeptideFoundationMultimodalV0380Config,
    PeptideFoundationMultimodalV0380Model, RetentionTimeObjective,
...
const V038_VERSION: u32 = 381;
const V038_OBJECTIVE: &str = "v0381_standardized_mobility_residual_conditioning";
const V038_ARCHITECTURE: &str = "v0.38.1-v0380-standardized-mobility-residual-conditioning";
const V038_MAX_STEPS_PER_EPOCH: usize = 1_536;
const V038_MAX_DEV_BATCHES: usize = 32;
const V038_MAX_HOLDOUT_BATCHES: usize = 32;
const V038_LOSS_CALIBRATION_RECORDS: usize = 4_096;
...
const V038_MAX_GRADIENT_NORM: f64 = 1.0;
const V038_MATERIAL_RAW_CCS_RATIO: f64 = 0.90;
const V038_MATERIAL_CONSENSUS_CCS_RATIO: f64 = 0.88;
const V038_RAW_OBJECTIVE_WEIGHT: f64 = 0.65;
const V038_CONSENSUS_OBJECTIVE_WEIGHT: f64 = 0.35;
const V038_RT_INVARIANCE_TOLERANCE: f64 = 5.0e-5;
const V038_MS2_INVARIANCE_TOLERANCE: f64 = 5.0e-5;
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 || args.len() > 11 {
        anyhow::bail!(
            "usage: foundation_train_multimodal_v0381 RUN_V0260.yaml OUTPUT_DIR PARENT_V0350_CHECKPOINT [max_epochs=10] [batch_size=96] [patience=3] [min_delta=0.002] [seed=20261038] [learning_rate=3e-5] [mode=train|finalize]"
        );
    }
    ...
    let finalize_only = match run_mode {
        ...
        "finalize" => true,
        other => {
            anyhow::bail!("unsupported v0.38.1 run mode {other:?}; expected train or finalize")
        }
    };

    if max_epochs == 0 || batch_size < 2 || patience == 0 {
        anyhow::bail!("v0.38.1 requires max_epochs>0, batch_size>=2, and patience>0");
    }
    if !(min_delta > 0.0 && min_delta.is_finite()) {
        anyhow::bail!("v0.38.1 min_delta must be positive and finite");
    }
    if !(learning_rate > 0.0 && learning_rate.is_finite()) {
        anyhow::bail!("v0.38.1 learning_rate must be positive and finite");
    }
        ...
        for name in ["initial", "best"] {
            ...
            for file in [
                ...
                if !directory.join(file).is_file() {
                    anyhow::bail!("v0.38.1 finalize is missing {:?}", directory.join(file));
                }
            }
        }
        if output_root.join("final").exists() {
            anyhow::bail!(
                "v0.38.1 final checkpoint already exists; HOLDOUT must not be consumed twice"
            );
        }
    } else if output_root.exists() {
        anyhow::bail!(
            "v0.38.1 TRAIN output directory already exists: {:?}",
            output_root
        );
    }

    #[cfg(feature = "cuda")]
    let device = Device::new_cuda(0).context("v0.38.1 requires a CUDA device")?;
    ...
    if mobility_train_indices.len() < batch_size || ccs_dev_indices.len() < batch_size {
        anyhow::bail!(
            "v0.38.1 requires at least one batch of CCS labels in TRAIN and DEV; observed train={} dev={} batch_size={batch_size}",
    ...
    if parent_metadata.version != 350 {
        anyhow::bail!(
            "v0.38.1 requires v0.35 metadata version 350, observed {}",
    ...
    {
        anyhow::bail!(
            "v0.38.1 requires the accepted v0.35 objective, observed {}",
            parent_metadata.objective
        );
    }
    if parent_metadata.completed_steps == 0 {
        anyhow::bail!("v0.38.1 refuses an unselected v0.35 baseline checkpoint");
    }
    parent_metadata.v0350_config.validate()?;
    let v0380_config =
        PeptideFoundationMultimodalV0380Config::fixed(parent_metadata.v0350_config.clone())?;
    let forward_config = v0380_config.forward().clone();
    ...
    if parent_metadata.corpus_fingerprint != current_corpus_fingerprint {
        anyhow::bail!("v0.38.1 corpus fingerprint differs from frozen v0.35 parent");
    }
    if parent_metadata.benchmark_manifest_fingerprint != current_benchmark_fingerprint {
        anyhow::bail!("v0.38.1 benchmark fingerprint differs from frozen v0.35 parent");
    }
        ...
        .chain(&holdout_indices)
    {
        ...
        if length > forward_config.max_sequence_len {
            anyhow::bail!(
                "v0.38.1 record {index} length {length} exceeds max_sequence_len={}",
    ...
    if train_supervision.examples.len() < batch_size {
        anyhow::bail!("v0.38.1 consensus TRAIN has fewer examples than batch_size");
    }
    ...
    if dev_consensus.is_empty() {
        anyhow::bail!("v0.38.1 DEV consensus set is empty");
    }
    ...
    let model = PeptideFoundationMultimodalV0380Model::new(v0380_config.clone(), vb)?;
    ...
    let calibration_count = V038_LOSS_CALIBRATION_RECORDS
    ...
    println!("v0381_version\tv0.38.1-standardized-mobility-residual-conditioning");
    println!("objective\t{V038_OBJECTIVE}");
    println!("architecture\t{V038_ARCHITECTURE}");
    ...
    println!(
        "mobility_residual_parameterization\tstandardized_residual_times_train_mobility_scale"
    );
    ...
    println!("optimizer_scope\tccs_forward_v0380.encoder+ccs_property_refinement_v0380+ccs_context_v0380_only");
    println!("optimizer_variable_count\t{}", optimizer.variable_count());
    println!("v0350_update_policy\texact_frozen_anchor");
    println!("ccs_representation_update_policy\ttrainable_clone_initialized_from_v0350");
    ...
    println!(
        "material_dev_gate\traw_ccs_ratio<={V038_MATERIAL_RAW_CCS_RATIO:.2}_and_consensus_ccs_ratio<={V038_MATERIAL_CONSENSUS_CCS_RATIO:.2}_and_rt_ms2_invariant"
    );
    println!("historical_validation_reused_for_v0381_selection\tNO");
    ...
    if finalize_only {
        ...
        if best_metadata.completed_steps == 0 {
            anyhow::bail!("v0.38.1 has no DEV improvement; refusing to consume HOLDOUT");
        }
        ...
        println!("v0381_finalize_dev_objective\t{objective:.8}");
        println!("v0381_finalize_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0381_finalize_consensus_ccs_ratio\t{consensus_ratio:.8}");
        if raw_ratio > V038_MATERIAL_RAW_CCS_RATIO
            || consensus_ratio > V038_MATERIAL_CONSENSUS_CCS_RATIO
        {
            anyhow::bail!(
                "v0.38.1 DEV result does not satisfy the fixed materiality gate; refusing HOLDOUT"
            );
        }

        println!("v0381_finalize_only\ttrue");
        println!(
            "v0381_finalize_best_step\t{}",
        ...
        println!(
            "v0381_holdout_raw_ccs\trecords={}\tmae={holdout_raw_ccs:.8}",
            holdout_ccs_indices.len()
        );
        println!(
            "v0381_holdout_consensus_ccs\tidentities={}\tmae={holdout_consensus_ccs:.8}",
        ...
        println!(
            "v0381_finalize_complete\tbest_step={}",
            best_metadata.completed_steps
        );
    ...
    let metadata = |completed_steps| V038Metadata {
        version: V038_VERSION,
        objective: V038_OBJECTIVE.into(),
        architecture: V038_ARCHITECTURE.into(),
        ...
        v0380_config: v0380_config.clone(),
    ...
    for epoch in 1..=max_epochs {
        ...
        println!(
            "v0381_epoch\tstage=start\tepoch={epoch}\tsteps={steps_per_epoch}\texamples={needed}"
        );
        ...
        for local_step in 0..steps_per_epoch {
            ...
            let update = optimizer.backward_step(&loss, Some(V038_MAX_GRADIENT_NORM))?;
            if global_step == 1 || global_step % 100 == 0 || local_step + 1 == steps_per_epoch {
                println!(
                    "v0381_train\tepoch={epoch}\tstep={global_step}\tepoch_step={}\tlr={:.8}\ttotal={loss_value:.6}\tgradient_norm={:.6}\tgradient_scale={:.6}",
                    local_step + 1,
        ...
        if improved {
            ...
            println!("v0381_best_checkpoint\tepoch={best_epoch}\tstep={best_step}\tdev_objective={best_objective:.8}\traw_ccs_ratio={raw_ratio:.8}\tconsensus_ccs_ratio={consensus_ratio:.8}");
        } else {
            stale_epochs += 1;
        }
        println!("v0381_epoch\tstage=complete\tepoch={epoch}\tstep={global_step}\tstale_epochs={stale_epochs}");
        if stale_epochs >= patience {
            stopped_early = true;
            println!("v0381_early_stop\tepoch={epoch}\tstep={global_step}\tpatience={patience}\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}");
            break;
        }
    }
    ...
    println!("historical_validation_reused_for_v0381_selection\tNO");
    println!("historical_test_consumed\tNO");
    println!("v0381_training_complete\tbest_epoch={best_epoch}\tbest_step={best_step}\tbest_dev_objective={best_objective:.8}\tstopped_early={stopped_early}");

    if best_step == 0 {
        println!("v0381_material_dev_gain\tNO");
        println!("v0381_finalize_required\tNO");
        println!("v0381_rethink_required\tYES");
    } else {
        ...
        let material = raw_ratio <= V038_MATERIAL_RAW_CCS_RATIO
            && consensus_ratio <= V038_MATERIAL_CONSENSUS_CCS_RATIO;
        println!("v0381_best_raw_dev_ccs_mae\t{best_raw_dev:.8}");
        println!("v0381_best_raw_ccs_ratio\t{raw_ratio:.8}");
        println!("v0381_best_consensus_dev_ccs_mae\t{best_consensus_dev:.8}");
        println!("v0381_best_consensus_ccs_ratio\t{consensus_ratio:.8}");
        println!(
            "v0381_rt_stretch_target_met\t{}",
            yes_no(
                best_protected_dev
                    .rt_mae_native
        ...
        println!(
            "v0381_ccs_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_TARGET_MAE_V0360)
        );
        println!(
            "v0381_ccs_stretch_target_met\t{}",
            yes_no(best_raw_dev <= FOUNDATION_CCS_STRETCH_TARGET_MAE_V0360)
        );
        if material {
            println!("v0381_material_dev_gain\tYES");
            println!("v0381_finalize_required\tYES");
            println!("v0381_rethink_required\tNO");
        } else {
            println!("v0381_material_dev_gain\tNO");
            println!("v0381_finalize_required\tNO");
            println!("v0381_rethink_required\tYES");
...
fn collect_mobility_groups(
    ...
    for &index in indices {
        let record = records
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.38.1 record index {index} outside corpus"))?;
        ...
        let source = provenance
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.38.1 missing provenance for record {index}"))?;
        groups
    ...
    for (identity, observations) in groups {
        ...
        for (family, mut views) in family_views {
            ...
            let representative = views
                ...
                .map(|entry| entry.2)
                .ok_or_else(|| anyhow::anyhow!("v0.38.1 family has no representative"))?;
        ...
        let representative_index = adjusted
            ...
            .map(|entry| entry.3)
            .ok_or_else(|| anyhow::anyhow!("v0.38.1 consensus identity has no representative"))?;
        total_weight += weight;
...
fn write_mobility_supervision_summary(
    ...
    fs::write(
        output_root.join("mobility_source_reliability_v0380.tsv"),
        source_lines,
    )?;
    let summary = format!(
    ...
    fs::write(
        output_root.join("mobility_consensus_supervision_v0380.tsv"),
        summary,
    )?;
    Ok(())
}
...
fn predicted_native_mobility_and_ccs(
    ...
    mobility_residual_scale_native: f64,
    train: bool,
) -> Result<(Tensor, Tensor)> {
    if !(mobility_residual_scale_native > 0.0 && mobility_residual_scale_native.is_finite()) {
        anyhow::bail!(
            "v0.38.1 mobility residual scale must be positive and finite; got {mobility_residual_scale_native}"
        );
    }
    let output = model.mobility_v0380_t(&batch.input, &batch.context, physics, train)?;
    ...
    // v0.38 emitted native 1/K0 directly while the normalized loss divided by
    // ~0.031, amplifying gradients before global clipping. v0.38.1 predicts a
    // dimensionless standardized residual and maps it back to native 1/K0 with
    // the fixed TRAIN-derived mobility scale.
    let mobility_residual_native = output
        .mobility_residual_native
        .affine(mobility_residual_scale_native, 0.0)?;
    let predicted_mobility = (&base_mobility + &mobility_residual_native)?;
...
fn mobility_consensus_loss(
    ...
    let owned = examples
        ...
        .map(|example| {
            records
                ...
                .cloned()
                .ok_or_else(|| anyhow::anyhow!("v0.38.1 representative index outside corpus"))
        })
    ...
    let (predicted_mobility, predicted_ccs) = predicted_native_mobility_and_ccs(
        model,
        &batch,
        &physics,
        normalization,
...
fn calibrate_mobility_residual_scales(
    ...
    for chunk in examples.chunks(batch_size) {
        ...
        let (predicted_mobility, predicted_ccs) =
            predicted_native_mobility_and_ccs(model, &batch, &physics, normalization, 1.0, false)?;
    ...
    if !(weight_sum > 0.0 && mobility_squared.is_finite() && ccs_squared.is_finite()) {
        anyhow::bail!("v0.38.1 TRAIN loss calibration has no finite weighted labels");
    }
...
fn weighted_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.38.1 weighted scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
    if !(scale > 0.0 && scale.is_finite()) {
        anyhow::bail!("v0.38.1 invalid loss scale {scale}");
    }
    let combined = mask
        .broadcast_mul(weight)?
...
fn weighted_scaled_pseudo_huber(
    ...
    if !(scale > 0.0 && scale.is_finite() && delta > 0.0 && delta.is_finite()) {
        anyhow::bail!("v0.38.1 invalid robust loss scale={scale} delta={delta}");
    }
    let combined = mask
        .broadcast_mul(weight)?
        .broadcast_as(prediction.dims())?;
...
fn evaluate_raw_ccs_indices(
    ...
    for chunk in indices.chunks(batch_size) {
        ...
        let (_, predicted_ccs) = predicted_native_mobility_and_ccs(
            model,
            &batch,
            &physics,
            normalization,
            mobility_residual_scale_native,
            false,
        )?;
    ...
    if count == 0 {
        anyhow::bail!("v0.38.1 raw CCS evaluation contains no labels");
    }
...
fn evaluate_consensus_targets(
    ...
    if count == 0 {
        anyhow::bail!("v0.38.1 consensus CCS evaluation contains no labels");
    }
    Ok(absolute_error / count as f64)
}
...
fn masked_scaled_mse(
    ...
    if prediction.dims() != target.dims() {
        anyhow::bail!(
            "v0.38.1 scalar loss shape mismatch: prediction {:?}, target {:?}",
            prediction.dims(),
            target.dims()
        );
    }
    if !(scale > 0.0 && scale.is_finite()) {
        anyhow::bail!("v0.38.1 invalid CCS loss scale {scale}");
    }
...
fn validate_rt_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        if (current - initial).abs() > V038_RT_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.38.1 changed frozen {label}: baseline={initial:.8} current={current:.8}"
            );
        }
    }
    Ok(())
...
fn validate_ms2_invariance(metrics: PropertyMetrics, baseline: PropertyMetrics) -> Result<()> {
    for (label, current, initial) in [
        ...
        let current = current.ok_or_else(|| anyhow::anyhow!("missing {label} metric"))?;
        let initial = initial.ok_or_else(|| anyhow::anyhow!("missing baseline {label} metric"))?;
        if (current - initial).abs() > V038_MS2_INVARIANCE_TOLERANCE {
            anyhow::bail!(
                "v0.38.1 changed frozen {label}: baseline={initial:.8} current={current:.8}"
            );
        }
...
fn load_v0350_parent_variables(
    ...
    let data = varmap
        .data()
        .lock()
        .map_err(|_| anyhow::anyhow!("v0.38.1 VarMap lock poisoned during warm start"))?;
    ...
    for (name, variable) in data.iter() {
        let parent_name = if name.starts_with("ccs_context_v0380.") {
            None
        } else if let Some(suffix) = name.strip_prefix("ccs_forward_v0380.") {
            Some(format!("forward_v0350.{suffix}"))
        } else if let Some(suffix) = name.strip_prefix("ccs_property_refinement_v0380.") {
        ...
        match tensors.get(&parent_name) {
            Some(tensor) => {
                if tensor.dims() != variable.as_tensor().dims() {
                    anyhow::bail!(
                        "v0.38.1 warm-start shape mismatch for {name} <- {parent_name}: parent {:?}, model {:?}",
                        tensor.dims(),
    ...
    if !missing.is_empty() {
        anyhow::bail!(
            "v0.35 parent is missing required v0.38.1 reused variables: {}",
            missing.join(", ")
        );
    }
    ...
    drop(data);
    if loaded == 0 || fresh == 0 {
        anyhow::bail!("v0.38.1 warm start is nonfunctional: loaded={loaded} fresh={fresh}");
    }
    Ok(WarmStartReport {
...
fn read_v038_metadata(checkpoint: &Path) -> Result<V038Metadata> {
    let path = checkpoint.join("metadata.yaml");
    serde_yaml::from_str(
        &fs::read_to_string(&path).with_context(|| format!("failed to read {path:?}"))?,
    )
    .with_context(|| format!("failed to parse v0.38.1 metadata {path:?}"))
}
...
fn validate_metadata(
    ...
    if metadata.version != V038_VERSION || metadata.objective != V038_OBJECTIVE {
        anyhow::bail!("v0.38.1 checkpoint identity mismatch");
    }
    ...
    {
        anyhow::bail!("v0.38.1 checkpoint has invalid mobility/CCS loss scale");
    }
    ...
    {
        anyhow::bail!("v0.38.1 checkpoint data/supervision provenance mismatch");
    }
    if metadata.v0380_config != *config {
        anyhow::bail!("v0.38.1 checkpoint architecture mismatch");
    }
    if metadata.batch_size != batch_size
        || metadata.seed != seed
        || metadata.train_steps != train_steps
    {
        anyhow::bail!("v0.38.1 checkpoint run-contract mismatch");
    }
    if (metadata.learning_rate - learning_rate).abs()
        > f64::EPSILON * 64.0 * learning_rate.abs().max(1.0)
    {
        anyhow::bail!("v0.38.1 checkpoint learning-rate mismatch");
    }
    Ok(())
}
...
fn feasible_validation_batches(
    ...
    if max_batches == 0 {
        anyhow::bail!("v0.38.1 {label} cannot form one full batch");
    }
    if config.validation_source_weights.is_empty() {
    ...
    for &index in validation_indices {
        let source = provenance
            .get(index)
            .ok_or_else(|| anyhow::anyhow!("v0.38.1 {label} provenance index out of bounds"))?;
    ...
    if !(total_weight > 0.0 && total_weight.is_finite()) {
        anyhow::bail!("v0.38.1 {label} validation weights are invalid");
    }
    ...
    anyhow::bail!("v0.38.1 {label} cannot satisfy source quotas for one full batch")
}
...
fn require_plan_records(label: &str, plan: &FoundationSamplePlan, expected: usize) -> Result<()> {
    if plan.indices.len() != expected {
        anyhow::bail!(
            "v0.38.1 {label} sample plan has {} records but {expected} are required",
            plan.indices.len()
        );
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v0381_affine_fit_recovers_linear_map() {
    ...
    #[test]
    fn v0381_consensus_prefers_reliable_source() {
    ...
    #[test]
    fn v0381_overlapping_views_collapse_to_project_families() {
    ...
    #[test]
    fn v0381_standardized_residual_maps_back_to_native_scale() {
        let device = Device::Cpu;
        let standardized = Tensor::from_vec(vec![2.0f32, -1.5], (2, 1), &device).unwrap();
        let native = standardized.affine(0.031, 0.0).unwrap();
        let values = native.to_vec2::<f32>().unwrap();
        assert!((values[0][0] - 0.062).abs() < 1.0e-6);
        assert!((values[1][0] + 0.0465).abs() < 1.0e-6);
    }

    #[test]
    fn v0381_epoch_order_is_deterministic_and_unique_within_cycle() {
        let examples = (0..32)
