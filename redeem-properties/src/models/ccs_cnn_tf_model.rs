...
impl candle_onnx_export::ToOnnx for CCSCNNTFModel {
    fn to_onnx(
        &self,
        ...
        let input = inputs
            .first()
            .cloned()
            .unwrap_or_else(|| crate::onnx_export::add_ccs_cnn_tf_input(ctx.graph, "input"));
        let output = crate::onnx_export::export_ccs_cnn_tf(&self.varmap, ctx, input)?;
        Ok(vec![output])
    }
}
