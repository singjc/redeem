...
use redeem_properties::models::{
    ...
    ms2_bert_model::MS2BertModel, rt_cnn_lstm_model::RTCNNLSTMModel,
...
pub fn run_training(config: &PropertyTrainConfig) -> Result<()> {
    ...
    let mut model: Box<dyn ModelInterface + Send + Sync> = match &config.checkpoint_file {
        ...
        None => match config.model_arch.as_str() {
            ...
            "ms2_bert" => Box::new(MS2BertModel::new_untrained(device.clone())?),
            _ => {
                return Err(anyhow::anyhow!(
                    "Unsupported model architecture: {}",
                    config.model_arch
    ...
    let mae_plot =
        plot_training_metric(&train_step_metrics, "mae", "MAE Over Steps", "Step", "MAE");
    ...
    // Section: Validation (inference diagnostics)
    {
        ...
        for (true_p, pred_p) in val_peptides.iter().zip(&inference_results) {
            ...
            if let Some((t, p)) = maybe_pair {
                ...
                let mc = if mod_sites.trim().is_empty() {
                    0
                } else {
                    mod_sites.split(';').filter(|s| !s.is_empty()).count()
                };
            ...
            for i in 0..top_n {
                ...
                let (t, p) =
                    if config.model_arch == "ccs_cnn_lstm" || config.model_arch == "ccs_cnn_tf" {
                        ...
                        (
                            true_p.retention_time.unwrap() as f64,
                            pred_p.retention_time.unwrap() as f64,
                        )
                    };
